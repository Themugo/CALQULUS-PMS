...
for (const name of functionNames) {
  ...
  const envVars = [...new Set([...source.matchAll(/Deno\.env\.get\(["']([A-Z0-9_]+)["']\)/g)].map((match) => match[1]))].sort();
...
const checklist = read(join(root, "PRODUCTION_CHECKLIST.md"));
...
const demoPanel = read(demoPanelPath);
...
for (const name of functionNames) {
  const source = read(join(functionsDir, name, "index.ts"));
  for (const match of source.matchAll(/Deno\.env\.get\(["']([A-Z0-9_]+)["']\)/g)) {
    referencedSupabaseEnv.add(match[1]);
  }
}
