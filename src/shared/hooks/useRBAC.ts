...
const SUBMANAGER_MAP: Record<SubmanagerCan, string> = {
...
const WEBHOST_MAP: Record<WebhostCan, string> = {
...
export const useRBAC = () => {
  ...
  const can = (permission: PermissionKey): boolean => {
    ...
    // Tenants and landlords have no administrative permissions
    ...
    if (webhostKey && isWebhost) {
      ...
      return !!(webhostPermissions as any)?.[webhostKey];
    }

    // Submanager checks
    const submanagerKey = SUBMANAGER_MAP[permission as SubmanagerCan];
    if (submanagerKey && isSubmanager) {
      return !!(submanagerPermissions as any)?.[submanagerKey];
    }
