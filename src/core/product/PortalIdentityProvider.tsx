...
import {
  DEFAULT_PORTAL_IDENTITIES,
  portalIdentityFromRow,
  type PortalIdentity,
} from "./portalIdentity";
...
export function portalFromPath(pathname: string): PortalId {
  ...
  if (pathname.startsWith("/tenant") || pathname.startsWith("/portal"))
    return "tenant";
...
export function PortalIdentityProvider({ children }: { children: ReactNode }) {
  ...
  const portalId =
    portalFromPath(location.pathname) ||
    portalFromAppRole(userRole?.role) ||
    "manager";
  const fallback = DEFAULT_PORTAL_IDENTITIES[portalId];
  const themeStorageKey = user?.id
    ? `calqulus-portal-theme:${user.id}:${portalId}`
    : `calqulus-portal-theme:guest:${portalId}`;
  const [themeMode, setThemeModeState] = React.useState<"portal" | "white">(
    () => {
      try {
        return localStorage.getItem(themeStorageKey) === "white"
          ? "white"
          : "portal";
      } catch {
        return "portal";
  ...
  React.useEffect(() => {
    try {
      setThemeModeState(
        localStorage.getItem(themeStorageKey) === "white" ? "white" : "portal",
      );
    } catch {
      setThemeModeState("portal");
    }
  }, [themeStorageKey]);
  ...
  const { data: identities = DEFAULT_PORTAL_IDENTITIES, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data: rows, error } = await (supabase.from as any)(
        "platform_portal_identities",
      ).select(
        "portal_id,display_name,short_name,tagline,primary_hex,background_image_url",
      );
  ...
  const palette = deriveBrandPalette(
    themeMode === "white" ? "#16324F" : identity.primaryHex,
  );
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      [
        ...
        "--portal-primary-muted",
        "--portal-primary-border",
        "--portal-primary-surface",
  ...
  const value = useMemo(
    ...
    [portalId, identity, identities, isLoading, themeMode, setThemeMode],
  );
  return (
    <PortalIdentityContext.Provider value={value}>
      {children}
