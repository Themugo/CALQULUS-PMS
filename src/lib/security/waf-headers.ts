 ...
 * SECURITY NOTE: 'unsafe-inline' for scripts is removed for XSS protection.
 * If runtime CSS injection is needed, use nonces or hashes instead.
 * See: https://content-security-policy.com/strict/
 */
export function getDefaultCSPDirectives(): CSPDirectives {
  return {
    ...
    'font-src': "'self' data:",
    'connect-src': "'self' https://*.supabase.co wss://*.supabase.co https://api.stripe.com https://*.sentry.io https://api.resend.com",
    'media-src': "'self' blob:",
    'object-src': "'none'",
    'frame-src': "'self'",
    'base-uri': "'self'",
