...
import {
  ...
  Banknote, PieChart, Activity, MessageSquare,
  Settings, BarChart3
} from 'lucide-react';
...
const LandlordDashboard = () => {
  ...
  // ── Fetch recent paid invoices for landlord properties ──────────
  const { data: recentPayments = [], isLoading: paymentsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from('invoices')
        .select(`id, amount, paid_date, invoice_number, tenant_id, tenants(name, unit, property)`)
        .in('tenants.property_id' as unknown as string, propIds)
        .eq('status', 'paid')
        .order('paid_date', { ascending: false })
        .limit(20);

      if (error) return [];
      return (data || []).map((inv: { id: string; amount: number; paid_date: string; invoice_number: string; tenants?: { name: string; unit: string; property: string } }) => ({
        id: inv.id,
        amount: Number(inv.amount),
        paid_date: inv.paid_date,
        invoice_number: inv.invoice_number,
        tenant_name: inv.tenants?.name ?? 'Tenant',
        property_name: inv.tenants?.property ?? 'Property',
        unit: inv.tenants?.unit ?? '—',
      })) as RecentPayment[];
    },
    enabled: !!user && userRole?.role === 'landlord' && properties.length > 0,
  });
        ...
        <Tabs defaultValue="properties">
          <TabsList className="mb-6 flex-wrap h-auto gap-1 p-1">
            ...
            <TabsTrigger value="payments" className="gap-1.5 text-xs">
              <Activity className="h-3.5 w-3.5" />Payment Activity
            </TabsTrigger>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          <div>
                            ...
                            <p className="font-medium text-sm">{prop.occupied}/{prop.units}</p>
                          </div>
                          <div>
                            <p className="text-xs text-muted-foreground">Occupancy</p>
                            <p className="font-medium text-sm">
                              {prop.units > 0 ? Math.round((prop.occupied / prop.units) * 100) : 0}%
                            </p>
                          </div>
                          <div>
          ...
          <TabsContent value="payments">
            <Card>
              <CardHeader>
                <CardTitle className="text-base">Recent tenant payments</CardTitle>
                <CardDescription>Payments collected across your properties</CardDescription>
              </CardHeader>
              <CardContent>
                {paymentsLoading ? (
                  ...
                  <div className="py-12 text-center text-muted-foreground">
                    ...
                    <p className="text-sm">No payment activity yet</p>
                  </div>
                ) : (
                  <Table>
                    <TableHeader>
                    ...
                    <TableBody>
                      {recentPayments.map(pmt => (
                        <TableRow key={pmt.id}>
                          <TableCell className="font-mono text-xs">{pmt.invoice_number}</TableCell>
                          <TableCell>{pmt.tenant_name}</TableCell>
                          <TableCell className="text-sm">
                            <span>{pmt.property_name}</span>
                            <span className="text-muted-foreground"> · Unit {pmt.unit}</span>
                          </TableCell>
                          <TableCell className="text-sm text-muted-foreground">
                            {pmt.paid_date ? format(new Date(pmt.paid_date), 'dd/MM/yy') : '—'}
                          </TableCell>
