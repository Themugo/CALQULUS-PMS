...
export function GoogleGlyph({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 48 48" className={className} aria-hidden>
      ...
      <path
        fill="#4CAF50"
        d="M24 44c5.5 0 10.4-2.1 14.1-5.5l-6.5-5.5C29.6 34.7 26.9 36 24 36c-5.3 0-9.6-3.1-11.3-7.6l-6.6 5.1C9.6 39.6 16.2 44 24 44z"
      />
      <path
        fill="#1976D2"
        d="M43.6 20.5H42V20H24v8h11.3c-.8 2.3-2.2 4.2-4.1 5.6l6.5 5.5C39.9 37.4 44 31.6 44 24c0-1.3-.1-2.7-.4-3.5z"
      />
    </svg>
  );
}
...
export function PortalLoginLayout({
  ...
  const resolvedBackgroundImage =
    identity?.backgroundImageUrl || backgroundImage;
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex min-h-screen flex-col">
        <header className="flex items-center justify-between gap-4 px-5 py-5 sm:px-8 lg:px-10 xl:px-14">
          <Link to={PUBLIC_ROUTES.home} aria-label="CALQULUS home">
            <BrandMark
              size="nav"
              showWordmark
              subtitleOverride={brand.descriptor}
              wordmarkOverride={brand.name}
          ...
          <div className="flex items-center gap-2">
            <Link
              to={PUBLIC_ROUTES.portalAccess}
              className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-semibold text-white/90 backdrop-blur-md transition hover:bg-white/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70"
            >
              Change portal
            </Link>
            <Link
          ...
          <div className="mx-auto grid w-full max-w-[1500px] items-center gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(390px,470px)] xl:gap-14">
            <section className="flex min-h-0 flex-col justify-center px-2 py-8 sm:min-h-[500px] sm:px-4 lg:min-h-[calc(100vh-122px)] lg:py-10">
              <div className="max-w-3xl">
                <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3.5 py-2 text-xs font-bold tracking-[0.24em] text-white/95 backdrop-blur-md">
                  <BadgeIcon className="h-4 w-4" aria-hidden />
                  {resolvedPortalName.toUpperCase()} PORTAL
                </div>
                <h1 className="mt-6 max-w-3xl font-heading text-[clamp(2.7rem,5.6vw,5.35rem)] font-semibold leading-[0.94] tracking-[-0.05em] text-white drop-shadow-[0_3px_18px_rgba(0,0,0,0.24)]">
                  {(headlineLines ?? [resolvedPortalName]).map((line) => (
                    <span key={line} className="block">
                      {line}
                    </span>
                  ))}
                </h1>
                <p className="mt-4 max-w-3xl font-heading text-[clamp(1.45rem,2.45vw,2.2rem)] font-medium leading-[1.1] tracking-[-0.03em] text-white/96">
                  {resolvedSlogan}
                </p>
                ...
                {trustLabel ? (
                  <div className="mt-6 flex w-fit items-center gap-2.5 rounded-xl border border-white/16 bg-black/10 px-4 py-3 text-xs font-semibold text-white/92 backdrop-blur-md">
                    <span
                  ...
                  <div className="mt-8 grid max-w-3xl gap-3 sm:grid-cols-3">
                    {features.map(({ icon: Icon, label, text }) => (
                      ...
                      >
                        ...
                        <p className="mt-1 text-sm leading-5.5 text-white/72">
                          {text}
                        </p>
                      </div>
                    ))}
            ...
            <aside
              className="relative flex items-center justify-center lg:py-8"
              aria-label={`${resolvedPortalName} sign in`}
            >
              <div
                className="absolute inset-x-6 inset-y-8 rounded-[38px] bg-white/8 blur-3xl"
                aria-hidden
              />
        ...
        <footer className="flex items-center justify-between gap-4 border-t border-white/12 bg-black/5 px-5 py-4 text-xs text-white/70 sm:px-8 lg:px-10 xl:px-14">
          <span>© {new Date().getFullYear()} CALQULUS Limited</span>
          <span className="font-medium text-white/80">
            {resolvedPortalName} workspace
          </span>
        </footer>
      </div>
    </div>
...
export function PortalLoginCard({
  ...
  const isKnownPortal = (
    value: string,
  ): value is "manager" | "landlord" | "tenant" | "agency" =>
    ["manager", "landlord", "tenant", "agency"].includes(value);
  const identity = isKnownPortal(portalLabel)
    ? identities[portalLabel]
    : undefined;
  const resolvedAccent = identity?.primaryHex || accentHex;
  ...
  return (
    ...
    >
      <h2 className="font-heading text-2xl font-bold tracking-tight text-foreground sm:text-[26px]">
        Welcome Back!
      </h2>
      <p className="mt-1 text-[15px] leading-6 text-muted-foreground">
        Sign in to your {portalLabel} portal
      </p>
      ...
      {footNote ? (
        <div className="mt-5 border-t border-border pt-5">{footNote}</div>
      ) : null}
    </section>
  );
}
