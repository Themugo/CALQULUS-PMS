...
export type AppRole = 'manager' | 'tenant' | 'webhost' | 'submanager' | 'landlord';
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const pickRoleForPath = useCallback((roles: UserRole[], pathname: string): UserRole => {
    ...
    const managerPaths = ['/', '/properties', '/tenants', '/billing', '/settings',
      ...
      '/platform-billing', '/water-billing', '/reports', '/activity', '/submanagers',
      '/communications', '/landlord'];
    if (managerPaths.some(p => pathname === p || pathname.startsWith(p + '/'))) {
      const m = byRole.get('manager'); if (m) return m;
      const s = byRole.get('submanager'); if (s) return s;
    }
    const priority: AppRole[] = ['manager', 'submanager', 'webhost', 'landlord', 'tenant'];
  ...
  const fetchUserRole = useCallback(async (userId: string, retryCount = 0, pathnameOverride?: string): Promise<UserRole | null> => {
    ...
    if (picked.role === 'webhost') fetchWebhostPermissions(userId).then(setWebhostPermissions);
    else setWebhostPermissions(null);
    ...
    return picked;
  }, [location.pathname, pickRoleForPath, fetchWebhostPermissions, fetchSubmanagerPermissions]);
  ...
  useEffect(() => {
        ...
        if (prev) {
          ...
          try {
            toast({
              ...
              description: "We're having trouble verifying your session. Some features may be unavailable until your connection improves.",
              variant: "destructive",
            });
          } catch { /* toast provider may not be mounted yet */ }
          return false;
        }
        return prev;
      });
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_, session) => {
      ...
      if (session?.user) {
        ...
        setUserRole(null); setWebhostPermissions(null); setSubmanagerPermissions(null); setLoading(false);
      }
    });
  ...
  useEffect(() => {
    ...
    const roleMatchesPath = (
      (userRole.role === 'manager'    && !location.pathname.startsWith('/portal') && !location.pathname.startsWith('/landlord/dashboard') && !location.pathname.startsWith('/webhost') && !location.pathname.startsWith('/submanager')) ||
      ...
      (userRole.role === 'submanager' && location.pathname.startsWith('/submanager'))
    );
    if (!roleMatchesPath) {
  ...
  const signUp = async (email: string, password: string, fullName: string, role: AppRole, tenantId?: string) => {
    ...
    if (data.user) {
      ...
      if (role === 'manager') supabase.functions.invoke('notify-new-manager-signup', { body: { managerEmail: email, managerName: fullName } }).catch(() => {});
    }
    return { error: null };
  };
  ...
  const signOut = async () => {
    ...
    setWebhostPermissions(null); setSubmanagerPermissions(null);
  };
