...
const TenantAuth = () => {
  ...
  // Load invitation if token is present - REQUIRED for signup
  useEffect(() => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authData.user) {
        // Route through create-tenant-account edge function so that:
        // 1. Unit status is updated to 'occupied'
        // 2. properties.occupied counter is recalculated
        // 3. Rent/deposit from invitation is stored on tenant record
        // 4. user_roles row is created correctly
        // 5. Activation token and welcome email are sent
        ...
        const { data: createResult, error: createError } = await supabase.functions.invoke(
          'create-tenant-account',
          {
            body: {
              ...
              property:     invitation?.property_name || null,
              property_id:  invitation?.property_id   || null,
              unit:         invitation?.unit           || null,
              manager_id:   invitation?.invited_by     || null,
              monthlyRent:  invitation?.monthly_rent  || null,
              depositAmount: invitation?.house_deposit
  ...
  // If no invitation token, show invitation required message
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-accent/20 via-background to-primary/10 px-4">
        <Card className="w-full max-w-md border-accent/20 bg-card/95 backdrop-blur-sm shadow-2xl">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl font-bold text-foreground">Invitation Required</CardTitle>
            <CardDescription className="text-muted-foreground mt-2">
              Tenant registration is by invitation only
            </CardDescription>
          </CardHeader>
          <CardContent className="text-center space-y-6">
            <div className="space-y-3 text-muted-foreground text-sm">
              <p>To create a tenant account, you need an invitation from your property manager or landlord.</p>
              <p>
