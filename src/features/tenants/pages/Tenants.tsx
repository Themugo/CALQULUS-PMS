...
const Tenants = () => {
  ...
  const fetchTenants = useCallback(async () => {
    ...
    if (error) {
      ...
      if (data && data.length > 0) {
        generateSignedUrls((data || []) as Tenant[]);
      }
    }
    setIsLoading(false);
  }, [user, toast]);
  ...
  const openStatement = (tenant: Tenant) => {
    setSelectedTenant(tenant);
    setIsStatementOpen(true);
  };

  // Filter tenants by status tab and search
  ...
  const TenantTable = ({ tenantList }: { tenantList: Tenant[] }) => (
            ...
            <div className="mt-6">
              <Tabs defaultValue="profile">
                ...
                <TabsContent value="profile">
                  <TenantProfilePanel tenant={selectedTenant as any} onUpdate={fetchTenants} />
                </TabsContent>
                ...
                <TabsContent value="payers">
                  <PaymentPayersManager
                    ...
                    unitId={(selectedTenant as any).unit_id}
                ...
                <TabsContent value="notices">
                  <TenantNoticeComposer tenant={selectedTenant as any} />
                </TabsContent>
