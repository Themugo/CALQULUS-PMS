...
const TenantProfilePanel: React.FC<TenantProfilePanelProps> = ({ tenant, onUpdate }) => {
  ...
  const Field = ({ label, value, field, type = 'text', disabled = false }: any) => (
    <div>
  ...
  const SelectField = ({ label, field, options }: any) => (
    <div>
      ...
      {editing ? (
        ...
        <p className="text-sm mt-0.5 font-medium">{options.find((o: any) => o.value === profile?.[field])?.label || profile?.[field] || <span className="text-muted-foreground">—</span>}</p>
      )}
    </div>
  );
        ...
        {unitLinks.length > 0 && (
          <TabsContent value="units" className="mt-4">
            <div className="space-y-3">
              {unitLinks.map((link: any) => (
                <Card key={link.id}>
                  <CardContent className="p-4 flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium">
                        {(link.units as any)?.label || (link.units as any)?.unit_number} — {(link.properties as any)?.name}
                      </p>
