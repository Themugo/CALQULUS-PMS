...
export default function AgencyLayout({ children, title, description, actions }: AgencyLayoutProps) {
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-background" {...portalSurfaceProps("agency")}>
  ...
  if (!isDevAccessEnabled() && (!user || userRole?.role !== "agency")) {
    return <Navigate to={AGENCY_LOGIN} replace />;
  }

  const isActive = (href: string) => {
    if (href === AGENCY_ROUTES.dashboard) return location.pathname === href;
    if (href === AGENCY_ROUTES.portfolio) {
      return (
        location.pathname === href ||
        location.pathname.startsWith("/agency/properties")
      );
    }
    if (href === AGENCY_ROUTES.clients) {
      return location.pathname === href || location.pathname.startsWith("/agency/landlords");
    }
  ...
  return (
    <DeskEmbedProvider recordsHome={AGENCY_ROUTES.portfolio} propertyBase="/agency/properties">
      <div className="min-h-screen bg-background text-foreground" {...portalSurfaceProps("agency")}>
        ...
        >
          ...
          <nav className="flex-1 space-y-0.5 overflow-y-auto px-2 py-4" aria-label="Agency">
            {NAV.map((item) => {
          ...
          <div className="flex-shrink-0 border-t border-border p-3">
            {user ? (
              <div className="mb-2 rounded-lg border border-border bg-muted/40 px-3 py-2">
                <p className="text-[9px] font-semibold uppercase tracking-wider text-muted-foreground">Agency</p>
