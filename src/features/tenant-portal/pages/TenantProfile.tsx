...
const TenantProfile = () => {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('tenants')
        .select('id, name, email, phone, photo_url, property, unit')
  ...
  const loadNotificationPreferences = useCallback(async () => {
    try {
      ...
      if (data) {
        setNotifications({
          emailNotifications: (data as any).email_enabled ?? true,
          paymentReminders:   (data as any).payment_reminders ?? true,
          leaseAlerts:        (data as any).lease_alerts ?? true,
          maintenanceUpdates: (data as any).maintenance_updates ?? true,
        });
      }
