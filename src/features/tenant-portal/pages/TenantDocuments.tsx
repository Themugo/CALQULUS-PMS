...
const TenantDocuments: React.FC = () => {
  ...
  const [renewalOpen, setRenewalOpen] = useState<any | null>(null);
  ...
  const submitRefRequest = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from('tenant_reference_requests').insert({
        ...
        manager_id:     (tenant as any)?.manager_id,
              ...
              {inspLoading ? <Skeleton className="h-24 w-full" /> : inspections.length === 0 ? (
                ...
                <div className="space-y-3">
                  {inspections.map((insp: any) => {
                    ...
                    const checklist = insp.checklist_items as any;
                    const allIssues = checklist ? [
                      ...(checklist.rooms || []).flatMap((r: any) => r.items.filter((i: any) => i.status === 'not_working' || i.status === 'needs_repair').map((i: any) => ({ ...i, room: r.room }))),
                      ...(checklist.utilities || []).filter((i: any) => i.status === 'not_working' || i.status === 'needs_repair').map((i: any) => ({ ...i, room: 'Utilities' })),
                      ...(checklist.fixtures || []).filter((i: any) => i.status === 'not_working' || i.status === 'needs_repair').map((i: any) => ({ ...i, room: 'Fixtures' })),
                    ] : [];
