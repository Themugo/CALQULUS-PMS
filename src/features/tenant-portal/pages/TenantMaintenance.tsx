          ...
          {selectedRequest && (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                {(selectedRequest as any).photos_urls?.length > 0 && (
                  <div>
                    <Label className="text-muted-foreground">Your photos</Label>
                    <div className="flex gap-2 mt-1 flex-wrap">
                      {(selectedRequest as any).photos_urls.map((url: string, i: number) => (
                ...
                {(selectedRequest as any).completion_photos?.length > 0 && (
                  <div>
                    <Label className="text-muted-foreground">Completion photos (from manager)</Label>
                    <div className="flex gap-2 mt-1 flex-wrap">
                      {(selectedRequest as any).completion_photos.map((url: string, i: number) => (
                        <a key={i} href={url} target="_blank" rel="noopener noreferrer">
                ...
                {(selectedRequest as any).resolution_notes && (
                  <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                    <Label className="text-green-700 text-xs">Manager resolution notes</Label>
                    <p className="mt-1 text-sm text-green-900">{(selectedRequest as any).resolution_notes}</p>
                  </div>
                )}
