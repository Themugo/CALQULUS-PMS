...
const TenantWaterPortal: React.FC = () => {
  ...
  const { data: tenantInfo } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Get water config for this unit/property
      const { data: waterConfig } = await supabase.from('water_billing_config')
        .select('*')
      ...
      // Get meter config
      const { data: meterConfig } = await supabase.from('unit_water_config')
        .select('*')
        .eq('unit_id', (tenant as any).unit_id)
        .maybeSingle();

      // Get water company info
      const provider = (waterConfig as any)?.water_provider;
      let waterCompany = null;
      if (provider) {
        const { data: co } = await supabase.from('kenya_water_companies')
  ...
  const { data: readings = [], isLoading: readingsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from('water_meter_readings')
        .select('*')
        .eq('unit_id', (tenantInfo.tenant as any).unit_id)
  ...
  const submitReading = useMutation({
    mutationFn: async () => {
      ...
      const wc = tenantInfo?.waterConfig as any;
      ...
      const { error } = await supabase.from('water_meter_readings').insert({
        ...
        manager_id:           (tenantInfo!.tenant as any).manager_id,
      ...
      await supabase.from('in_app_notifications').insert({
        user_id:        (tenantInfo!.tenant as any).manager_id,
        manager_id:     (tenantInfo!.tenant as any).manager_id,
        title:          'Meter reading submitted',
        body:           `${(tenantInfo!.tenant as any).name} submitted a water meter reading for Unit ${(tenantInfo!.tenant as any).unit}: ${curr} m³ (consumption: ${consumption} m³)`,
        type:           'info',
        source:         'system',
  ...
  const wc = tenantInfo?.waterConfig as any;
  const waterCompany = tenantInfo?.waterCompany as any;
  const meterNo = (tenantInfo?.meterConfig as any)?.meter_number ?? wc?.meter_number ?? 'Not assigned';
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                ) : (waterCompany as any)?.payment_note ? (
                  <p className="text-xs text-amber-700">{(waterCompany as any).payment_note}</p>
                ) : null}
              </div>
            </div>
