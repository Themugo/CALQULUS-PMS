...
const TenantPaymentSchedule: React.FC = () => {
  ...
  // All invoices (to show payment timeline)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        ...
        <div className="rounded-lg border border-border p-3 bg-muted/20 col-span-2 sm:col-span-1">
          ...
          <p className="text-xl font-semibold">{schedules.filter((s: any) => s.status === 'active').length}</p>
        </div>
      </div>

      {/* Active instalment plans */}
      {schedules.filter((s: any) => s.status === 'active').map((schedule: any) => {
      ...
      {schedules.filter((s: any) => s.status === 'active').length === 0 && (
      ...
      {schedules.filter((s: any) => s.status === 'completed').length > 0 && (
        <Card className="opacity-75">
          ...
          <CardContent>
            {schedules.filter((s: any) => s.status === 'completed').map((s: any) => (
