  ...
  const { data: details, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (tenantId) {
        ...
        if (data) {
          return {
            monthly_rent:    (data as any).monthly_rent,
            house_deposit:   (data as any).deposit_amount,
            deposit_balance: (data as any).deposit_balance,
            total_deposit:   (data as any).deposit_amount,
            other_charges:   (data as any).other_charges,
            other_charges_desc: (data as any).other_charges_description,
            tenancy_type:    'standard',
            paybill_number:  null,
            account_reference: (data as any).unit,
            payment_day:     1,
          } as any;
        }
      }
