...
function Trust({ config }: { config: PublicSiteConfig["trust"] }) {
  ...
  return <section id="trust" className="bg-background py-3 sm:py-4"><div className={CONTAINER}><div className="grid gap-2.5 lg:grid-cols-[1.7fr_1fr]"><div className="rounded-2xl border border-border bg-[linear-gradient(135deg,#f5fbf8,#ffffff)] shadow-[0_6px_20px_rgba(16,42,67,0.06)] px-4 py-4 sm:px-5"><p className="font-heading text-[11px] font-bold tracking-[0.2em] text-primary">{config.eyebrow}</p><h2 className="mt-1 font-heading text-xl font-semibold tracking-[-0.03em] text-navy-deep">{config.title}</h2><p className="mt-1 max-w-3xl text-[11px] leading-5 text-muted-foreground">{config.copy}</p><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-4 xl:grid-cols-8">{logos.map((logo) => <NavLink key={logo.id} href={logo.href} className="flex h-10 min-w-0 items-center justify-center rounded-lg border border-border bg-white px-2 text-[12px] font-bold text-muted-foreground grayscale transition hover:grayscale-0">{logo.image ? <img src={logo.image} alt={logo.name} className="max-h-5 max-w-full object-contain"/> : logo.name}</NavLink>)}</div></div><div className="rounded-2xl border border-border bg-white px-4 py-4 shadow-[0_6px_20px_rgba(16,42,67,0.06)] sm:px-5"><p className="text-[18px] leading-6 text-primary">❝</p><p className="mt-1 max-h-[58px] overflow-hidden text-[12px] leading-5 text-navy-deep [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:4]">{config.quote}</p><div className="mt-3 flex items-center gap-2.5"><div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full bg-success/15 text-sm font-bold text-primary">{config.avatar ? <img src={config.avatar} alt="" className="h-full w-full object-cover"/> : config.person.charAt(0)}</div><div><p className="text-[12px] font-semibold text-navy-deep">{config.person}</p><p className="text-[11px] text-muted-foreground">{config.role}</p></div></div></div></div></div></section>;
}
