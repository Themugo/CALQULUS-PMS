...
import { ArrowRight, BarChart3, BriefcaseBusiness, Building2, ChevronLeft, ChevronRight, Clock3, Cloud, FileStack, Home, Landmark, Search, ShieldCheck, Sparkles, TrendingUp, Users, Wrench, Leaf, Settings2 } from "lucide-react";
...
const PORTAL_ICONS = { agency: BriefcaseBusiness, manager: Settings2, landlord: TrendingUp, tenant: Home } as const;
const PORTAL_GRADIENTS = {
  agency: "from-[#0F2A8F] via-[#123FB7]/92 to-transparent",
                ...
                >
                  <div className="relative z-10 flex min-w-0 flex-col justify-center overflow-hidden bg-[linear-gradient(135deg,#ffffff_0%,#f7fbff_58%,#edf5fb_100%)] px-5 py-7 sm:px-8 sm:py-8 lg:px-10 xl:px-12">
                    <div className="min-w-0 max-w-[540px]">
                      <p className="text-[10px] font-semibold tracking-[0.3em] text-primary sm:text-[11px]">
                        {slide.eyebrow}
                      </p>
                      <div className="mt-4 flex items-start gap-3">
                        ...
                        <div className="hidden shrink-0 pt-3 text-right sm:block" aria-hidden>
                          ...
                          <div className="mt-1 h-1.5 w-14 rounded-full bg-primary/75" />
                        </div>
                      </div>
                  ...
                  >
                    ...
                    <div className="relative flex h-full items-center gap-3 px-3.5 py-2.5">
                      <div className="min-w-0 flex-1">
                        <span className="inline-flex rounded-md bg-[#123FB7] px-2 py-0.5 text-[8px] font-bold tracking-[0.16em] text-white">
                          {promo.label}
                        </span>
...
function Trust({ config }: { config: PublicSiteConfig["trust"] }) {
  ...
  return <section id="trust" className="bg-background py-3 sm:py-4"><div className={CONTAINER}><div className="grid gap-2.5 lg:grid-cols-[1.7fr_1fr]"><div className="rounded-2xl border border-border bg-[linear-gradient(135deg,#F5F8FF,#FFFFFF)] shadow-[0_6px_20px_rgba(16,42,67,0.06)] px-4 py-4 sm:px-5"><p className="text-[10px] font-bold tracking-[0.22em] text-primary">{config.eyebrow}</p><h2 className="mt-1 font-heading text-xl font-semibold tracking-[-0.03em] text-navy-deep">{config.title}</h2><p className="mt-1 max-w-3xl text-[10px] leading-4.5 text-muted-foreground">{config.copy}</p><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-4 xl:grid-cols-8">{logos.map((logo) => <NavLink key={logo.id} href={logo.href} className="flex h-10 min-w-0 items-center justify-center rounded-lg border border-border bg-white px-2 text-[10px] font-bold text-muted-foreground grayscale transition hover:grayscale-0">{logo.image ? <img src={logo.image} alt={logo.name} className="max-h-5 max-w-full object-contain"/> : logo.name}</NavLink>)}</div></div><div className="rounded-2xl border border-border bg-white px-4 py-4 shadow-[0_6px_20px_rgba(16,42,67,0.06)] sm:px-5"><p className="text-[18px] leading-6 text-primary">❝</p><p className="mt-1 max-h-[58px] overflow-hidden text-[11px] leading-4.5 text-navy-deep [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:4]">{config.quote}</p><div className="mt-3 flex items-center gap-2.5"><div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full bg-primary/10 text-sm font-bold text-primary">{config.avatar ? <img src={config.avatar} alt="" className="h-full w-full object-cover"/> : config.person.charAt(0)}</div><div><p className="text-[10px] font-semibold text-navy-deep">{config.person}</p><p className="text-[9px] text-muted-foreground">{config.role}</p></div></div></div></div></div></section>;
}

function CTA({ config }: { config: PublicSiteConfig["cta"] }) {
  return <section id="cta" className="bg-background py-2 sm:py-3"><div className={CONTAINER}><div className="relative overflow-hidden rounded-2xl bg-[linear-gradient(135deg,#0B1F78_0%,#123FB7_58%,#0B2B7A_100%)] px-4 py-5 text-white shadow-[0_12px_34px_rgba(18,63,183,0.18)] sm:px-7"><div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_82%_18%,rgba(255,255,255,0.14),transparent_34%),radial-gradient(circle_at_15%_100%,rgba(125,159,245,0.14),transparent_30%)]"/><div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0 max-w-2xl"><p className="text-[10px] font-bold tracking-[0.22em] text-white/80">{config.eyebrow}</p><h2 className="mt-1 font-heading text-xl font-semibold leading-[1.12] tracking-[-0.02em] text-white sm:text-2xl">{config.title}</h2><p className="mt-1 max-w-xl text-[10px] leading-4 text-white/78 sm:text-xs">{config.copy}</p></div><div className="flex shrink-0 flex-wrap gap-2"><Button asChild className="h-9 rounded-lg bg-white px-4 text-xs font-bold text-[#0B1F78] shadow-sm hover:bg-white/90"><Link to={config.primaryHref}>{config.primaryLabel}<ArrowRight className="ml-1 h-3.5 w-3.5"/></Link></Button><Button asChild variant="outline" className="h-9 rounded-lg border-white/35 bg-white/10 px-4 text-xs font-semibold text-white hover:bg-white/18 hover:text-white"><NavLink href={config.secondaryHref}>{config.secondaryLabel}</NavLink></Button></div></div></div></div></section>;
}
