...
const PORTAL_GRADIENTS = {
  agency: "from-[#0B2B7A] via-[#123FB7]/94 to-transparent",
...
function AdCard({ ad, className = "" }: { ad: PublicSiteMarketingAd; className?: string }) {
  ...
  return (
    <NavLink
      href={ad.href}
      className={`group relative block overflow-hidden rounded-xl border border-white/50 bg-white/96 shadow-[0_14px_34px_rgba(16,42,67,0.16)] backdrop-blur-md ${sizeClass} ${className}`}
    >
      ...
      <div className="relative flex h-full items-center gap-3 px-4 py-3">
        <div className="min-w-0 flex-1">
          <span className="inline-flex rounded-md bg-primary px-2 py-0.5 text-[9px] font-bold tracking-[0.16em] text-white">{ad.label}</span>
          <p className="mt-1.5 line-clamp-2 text-sm font-bold leading-4.5 text-navy-deep">{ad.title}</p>
          <p className="mt-1 line-clamp-2 text-[11px] leading-4 text-muted-foreground">{ad.copy}</p>
        </div>
...
function Hero({ config }: { config: PublicSiteConfig }) {
  ...
  return (
    ...
    >
          ...
          <div className="relative h-[350px] sm:h-[370px] xl:h-[390px]">
            {slides.map((slide, index) => {
              ...
              return (
                <article
                  ...
                  className={`absolute inset-0 grid lg:grid-cols-[1.04fr_1.25fr] transition-all duration-1000 ease-in-out ${
                    activeSlide
                ...
                >
                  <div className="relative z-10 flex min-w-0 flex-col justify-center overflow-hidden bg-[linear-gradient(135deg,#ffffff_0%,#f7fbff_58%,#edf5fb_100%)] px-5 py-7 sm:px-8 sm:py-8 lg:px-10 xl:px-12">
                    <div className="min-w-0 max-w-[540px]">
                      <p className="font-heading text-[11px] font-semibold tracking-[0.28em] text-success sm:text-xs">
                        {slide.eyebrow}
                      </p>
                      ...
                      <p className="mt-4 max-w-xl text-[14px] leading-6 text-muted-foreground sm:text-[15px]">
                        {slide.copy}
                      </p>
                      <div className="mt-5 flex flex-wrap gap-2.5">
                        ...
                        <Button
                          ...
                          tabIndex={activeSlide ? 0 : -1}
                          variant="outline"
                          className="h-10 rounded-xl border-primary/20 bg-white px-5 text-[15px] font-semibold text-navy-deep hover:bg-muted"
                        >
                ...
                {promos.map((promo) => (
                  ...
                  >
                    ...
                    <div className="relative flex h-full items-center gap-3 px-3.5 py-2.5">
                      <div className="min-w-0 flex-1">
                        <span className="inline-flex rounded-md bg-[#197A60] px-2 py-0.5 text-[12px] font-bold tracking-[0.16em] text-white">
                          {promo.label}
                        </span>
                        <p className="mt-1 truncate text-[12px] font-semibold text-navy-deep">{promo.title}</p>
                        <p className="truncate text-[11px] text-muted-foreground">{promo.copy}</p>
                      </div>
            ...
            {slides.length > 1 ? (
              <div className="absolute bottom-12 right-4 z-30 flex items-center gap-1.5 sm:right-6">
                ...
                <span aria-live="polite" className="rounded-full bg-navy-deep/35 px-2 py-1 text-[12px] font-semibold text-white backdrop-blur">
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              {(pills.length ? pills : DEFAULT_PUBLIC_SITE_CONFIG.hero.pills).map((pill) => {
                ...
                return (
                  <span
                    key={pill.id}
                    className="inline-flex items-center gap-1.5 rounded-full border border-border bg-background px-2.5 py-1 text-[12px] font-semibold text-muted-foreground"
                  >
      ...
      {overlayAd(config.marketingAds, "hero") ? <div className="pointer-events-auto absolute z-40 hidden w-[270px] md:block lg:w-[300px]" style={{ right: "1.5rem", top: "1.5rem" }}><AdCard ad={overlayAd(config.marketingAds, "hero")!}/></div> : null}
    </section>
  );
}

function SectionHeading({ eyebrow, title, copy, action }: { eyebrow?: string; title: string; copy?: string; action?: ReactNode }) {
  return <div className="mb-3 flex min-w-0 items-end justify-between gap-3"><div className="min-w-0"><p className="font-heading text-[11px] font-bold tracking-[0.22em] text-primary sm:text-xs">{eyebrow}</p><h2 className="mt-1 max-w-3xl font-heading text-[clamp(1.7rem,2.7vw,2.25rem)] font-semibold leading-[1.08] tracking-[-0.045em] text-navy-deep">{title}</h2>{copy ? <p className="mt-1 max-w-2xl text-[13px] leading-5.5 text-muted-foreground sm:text-sm">{copy}</p> : null}</div>{action ? <div className="shrink-0">{action}</div> : null}</div>;
}
...
function PropertyTypes({ items, ads = [] }: { items: PublicSiteConfig["propertyTypes"]; ads?: PublicSiteMarketingAd[] }) {
  ...
  return <section id="property-types" className="bg-background py-3 sm:py-4"><div className={CONTAINER}><SectionHeading eyebrow="EXPLORE PROPERTY TYPES" title="Different properties. Smarter management." action={<div className="hidden items-center gap-1.5 sm:flex"><button aria-label="Previous property types" className="flex h-8 w-8 items-center justify-center rounded-full border border-border bg-white text-navy-deep"><ChevronLeft className="h-4 w-4" /></button><button aria-label="Next property types" className="flex h-8 w-8 items-center justify-center rounded-full border border-border bg-white text-navy-deep"><ChevronRight className="h-4 w-4" /></button><NavLink href="#property-types" className="ml-1 rounded-full border border-border bg-white px-3 py-1.5 text-[12px] font-semibold text-navy-deep">View All Types <ArrowRight className="ml-1 inline h-3 w-3" /></NavLink></div>} /><div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-4">{visible.map((item) => { const Icon = PROPERTY_ICONS[item.icon]; const replacement = replacementAd(ads, item.id); if (replacement) return <AdCard key={item.id} ad={replacement} className="min-h-[168px]"/>; return <NavLink key={item.id} href={item.href} className="group min-w-0 overflow-hidden rounded-xl border border-border bg-card shadow-[0_6px_18px_rgba(16,42,67,0.06)] transition hover:-translate-y-0.5 hover:shadow-[0_12px_26px_rgba(16,42,67,0.12)]"><div className="relative h-[96px] overflow-hidden sm:h-[108px]"><img src={img(item.image, PROPERTY_IMAGES.residential)} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105"/><div className="absolute inset-0 bg-gradient-to-t from-navy-deep/50 to-transparent"/></div><div className="relative flex min-h-[64px] items-center gap-3 px-3 py-2.5"><span className="-mt-9 flex h-11 w-11 shrink-0 items-center justify-center rounded-full border-4 border-white bg-white text-success shadow-sm"><Icon className="h-5 w-5" /></span><div className="min-w-0 flex-1"><h3 className="truncate font-heading text-[15px] font-semibold text-navy-deep">{item.title}</h3><p className="truncate text-[11px] text-muted-foreground">{item.description}</p></div><span className="ml-auto flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-success/40 text-success"><ArrowRight className="h-3.5 w-3.5" /></span></div></NavLink>; })}</div></div></section>;
}
...
function Portals({ items, ads = [] }: { items: PublicSiteConfig["portals"]; ads?: PublicSiteMarketingAd[] }) {
  ...
  return <section id="portals" className="bg-[#F2F6FB] py-4 sm:py-5"><div className={CONTAINER}><SectionHeading eyebrow="ACCESS YOUR PORTAL" title="Choose your portal to get started." copy="Different needs. One connected property platform."/><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{visible.map((item) => {
    const Icon = PORTAL_ICONS[item.id];
    const accent = PORTAL_GRADIENTS[item.id];
...
function WhyChoose({ config, ads = [] }: { config: PublicSiteConfig["why"]; ads?: PublicSiteMarketingAd[] }) {
  return <section id="why" className="bg-background py-2 sm:py-3"><div className={CONTAINER}><div className="flex min-w-0 flex-col gap-3 rounded-2xl border border-border/70 bg-[linear-gradient(90deg,#ffffff,#f5fbf8)] px-3.5 py-3 sm:flex-row sm:items-center sm:px-4"><div className="w-full min-w-0 sm:max-w-[285px]"><p className="font-heading text-[11px] font-bold tracking-[0.2em] text-primary">{config.eyebrow}</p><h2 className="mt-1 font-heading text-lg font-semibold leading-tight tracking-[-0.03em] text-navy-deep">{config.title}</h2>{config.copy ? <p className="mt-1 text-[11px] text-muted-foreground">{config.copy}</p> : null}</div><div className="grid min-w-0 flex-1 grid-cols-2 gap-2 xl:grid-cols-4">{config.cards.filter((card) => card.enabled).map((card) => { const Icon = WHY_ICONS[card.icon]; const replacement = replacementAd(ads, card.id); if (replacement) return <AdCard key={card.id} ad={replacement} className="min-h-[88px]"/>; return <div key={card.id} className="flex min-w-0 items-start gap-2 rounded-xl bg-white/90 px-2.5 py-2 shadow-[0_4px_14px_rgba(16,42,67,0.06)]"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-success/10 text-success"><Icon className="h-4 w-4"/></span><div className="min-w-0"><h3 className="truncate text-xs font-bold text-navy-deep">{card.title}</h3><p className="mt-0.5 overflow-hidden text-[12px] leading-4 text-muted-foreground [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:2]">{card.copy}</p></div></div>; })}</div></div></div></section>;
}

function Featured({ items, ads = [] }: { items: PublicSiteConfig["featured"]; ads?: PublicSiteMarketingAd[] }) {
  const visible = items.filter((item) => item?.enabled);
  return <section id="featured" className="bg-muted/10 py-3 sm:py-4"><div className={CONTAINER}><SectionHeading eyebrow="FEATURED PROPERTIES" title="Discover premium properties across every sector." action={<div className="hidden items-center gap-1.5 sm:flex"><button className="flex h-8 w-8 items-center justify-center rounded-full border border-border bg-white"><ChevronLeft className="h-4 w-4" /></button><button className="flex h-8 w-8 items-center justify-center rounded-full border border-border bg-white"><ChevronRight className="h-4 w-4" /></button><NavLink href="#featured" className="ml-1 rounded-full border border-border bg-white px-3 py-1.5 text-[12px] font-semibold text-navy-deep">View All Properties <ArrowRight className="ml-1 inline h-3 w-3" /></NavLink></div>} />{visible.length ? <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-4">{visible.map((item) => { const replacement = replacementAd(ads, item.id); if (replacement) return <AdCard key={item.id} ad={replacement} className="min-h-[220px]"/>; return <NavLink key={item.id} href={item.href} className="group overflow-hidden rounded-xl border border-border bg-card shadow-[0_6px_18px_rgba(16,42,67,0.06)] transition hover:-translate-y-0.5 hover:shadow-[0_12px_26px_rgba(16,42,67,0.12)]"><div className="relative h-[118px] overflow-hidden"><img src={img(item.image, PROPERTY_IMAGES.residential)} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105"/><span className="absolute right-2 top-2 max-w-[45%] truncate rounded-md bg-white/95 px-2 py-1 shadow-sm text-[11px] font-bold text-navy-deep">{item.price}</span></div><div className="min-h-[96px] p-3"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-[12px] font-bold uppercase tracking-[0.12em] text-primary">{item.eyebrow}</p><h3 className="truncate mt-0.5 font-heading text-[15px] font-semibold text-navy-deep">{item.title}</h3></div><span className="shrink-0 rounded-full bg-success/10 px-2 py-1 text-[12px] font-bold text-primary">Available</span></div><p className="truncate mt-1 text-[11px] text-muted-foreground">{item.location}</p><div className="mt-2 flex min-w-0 items-center justify-between gap-2 text-[11px] text-muted-foreground"><span className="min-w-0 truncate inline-flex items-center gap-1"><Home className="h-3 w-3 text-primary"/>{item.detail}</span><ArrowRight className="h-3.5 w-3.5 text-primary transition group-hover:translate-x-1"/></div></div></NavLink>; })}</div> : <div className="rounded-xl border border-dashed border-border bg-card p-5 text-center text-xs text-muted-foreground">Featured listings will appear here when published in Public Site Studio.</div>}</div></section>;
}
...
function Trust({ config, ads = [] }: { config: PublicSiteConfig["trust"]; ads?: PublicSiteMarketingAd[] }) {
  ...
  return <section id="trust" className="relative bg-background py-4 sm:py-5"><div className={CONTAINER}><div className="grid gap-3 lg:grid-cols-[1.75fr_1fr]">
    <div className="rounded-2xl border border-[#CFE0EF] bg-[linear-gradient(135deg,#F4F8FC_0%,#FFFFFF_48%,#F3FAF7_100%)] px-4 py-5 shadow-[0_8px_24px_rgba(16,42,67,0.07)] sm:px-6">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-heading text-[11px] font-bold tracking-[0.22em] text-primary">{config.eyebrow}</p><h2 className="mt-1 font-heading text-[clamp(1.45rem,2.3vw,1.9rem)] font-semibold tracking-[-0.035em] text-navy-deep">{config.title}</h2><p className="mt-1 max-w-3xl text-[12px] leading-5 text-muted-foreground sm:text-sm">{config.copy}</p></div><span className="inline-flex items-center rounded-full border border-primary/15 bg-primary/8 px-3 py-1.5 text-[10px] font-bold tracking-[0.14em] text-primary">PARTNERSHIPS · ASSURANCE</span></div>
      <div className="mt-4 grid grid-cols-[repeat(auto-fit,minmax(120px,1fr))] gap-2">{logos.map((logo) => { const replacement = replacementAd(ads, logo.id); if (replacement) return <AdCard key={logo.id} ad={replacement} className="min-h-[64px]"/>; return <NavLink key={logo.id} href={logo.href} className="group flex min-h-[64px] min-w-0 items-center justify-center rounded-xl border border-[#D7E2EC] bg-white px-3 py-3 shadow-[0_4px_12px_rgba(16,42,67,0.045)] transition hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-[0_8px_18px_rgba(16,42,67,0.08)]">{logo.image ? <img src={logo.image} alt={logo.name} className="max-h-9 max-w-[92%] object-contain transition group-hover:scale-[1.02]"/> : <span className="truncate text-center font-heading text-sm font-bold tracking-tight text-navy-deep">{logo.name}</span>}</NavLink>; })}</div>
      <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-[10px] font-medium text-muted-foreground"><span>Organization marks</span><span>•</span><span>Partner placements</span><span>•</span><span>Public trust content</span></div>
    </div>
    <div className="rounded-2xl border border-[#D7E2EC] bg-white px-4 py-5 shadow-[0_8px_24px_rgba(16,42,67,0.07)] sm:px-6"><p className="text-2xl leading-none text-primary">❝</p><p className="mt-2 max-h-[88px] overflow-hidden text-[13px] leading-5 text-navy-deep sm:text-sm">{config.quote}</p><div className="mt-4 flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full bg-primary/10 text-sm font-bold text-primary">{config.avatar ? <img src={config.avatar} alt="" className="h-full w-full object-cover"/> : config.person.charAt(0)}</div><div className="min-w-0"><p className="truncate text-xs font-semibold text-navy-deep">{config.person}</p><p className="truncate text-[11px] text-muted-foreground">{config.role}</p></div></div></div>
  </div></div></section>;
}

function CTA({ config }: { config: PublicSiteConfig["cta"] }) {
  return <section id="cta" className="bg-background py-2 sm:py-3"><div className={CONTAINER}><div className="relative overflow-hidden rounded-2xl bg-[linear-gradient(135deg,#0B2B7A_0%,#123FB7_58%,#07185E_100%)] px-4 py-5 text-white shadow-[0_12px_34px_rgba(18,63,183,0.18)] sm:px-7"><div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_82%_18%,rgba(255,255,255,0.14),transparent_34%),radial-gradient(circle_at_15%_100%,rgba(118,217,194,0.12),transparent_30%)]"/><div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0 max-w-2xl"><p className="text-[12px] font-bold tracking-[0.22em] text-white/80">{config.eyebrow}</p><h2 className="mt-1 font-heading text-xl font-semibold leading-[1.12] tracking-[-0.02em] text-white sm:text-2xl">{config.title}</h2><p className="mt-1 max-w-xl text-[12px] leading-5 text-white/78 sm:text-xs">{config.copy}</p></div><div className="flex shrink-0 flex-wrap gap-2"><Button asChild className="h-9 rounded-lg bg-white px-4 text-xs font-bold text-[#0B2B7A] shadow-sm hover:bg-white/90"><Link to={config.primaryHref}>{config.primaryLabel}<ArrowRight className="ml-1 h-3.5 w-3.5"/></Link></Button><Button asChild variant="outline" className="h-9 rounded-lg border-white/35 bg-white/10 px-4 text-xs font-semibold text-white hover:bg-white/18 hover:text-white"><NavLink href={config.secondaryHref}>{config.secondaryLabel}</NavLink></Button></div></div></div></div></section>;
}
