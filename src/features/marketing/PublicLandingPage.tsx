...
function CTA({ config }: { config: PublicSiteConfig["cta"] }) {
  return <section id="cta" className="bg-background py-2 sm:py-3"><div className={CONTAINER}><div className="relative overflow-hidden rounded-2xl bg-[linear-gradient(135deg,#006B5B_0%,#087F78_58%,#063F3A_100%)] px-4 py-5 text-white shadow-[0_12px_34px_rgba(0,95,82,0.18)] sm:px-7"><div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_82%_18%,rgba(255,255,255,0.14),transparent_34%),radial-gradient(circle_at_15%_100%,rgba(118,217,194,0.12),transparent_30%)]"/><div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0 max-w-2xl"><p className="text-[10px] font-bold tracking-[0.22em] text-white/80">{config.eyebrow}</p><h2 className="mt-1 font-heading text-xl font-semibold leading-[1.12] tracking-[-0.02em] text-white sm:text-2xl">{config.title}</h2><p className="mt-1 max-w-xl text-[10px] leading-4 text-white/78 sm:text-xs">{config.copy}</p></div><div className="flex shrink-0 flex-wrap gap-2"><Button asChild className="h-9 rounded-lg bg-white px-4 text-xs font-bold text-[#0C3F3A] shadow-sm hover:bg-white/90"><Link to={config.primaryHref}>{config.primaryLabel}<ArrowRight className="ml-1 h-3.5 w-3.5"/></Link></Button><Button asChild variant="outline" className="h-9 rounded-lg border-white/35 bg-white/10 px-4 text-xs font-semibold text-white hover:bg-white/18 hover:text-white"><NavLink href={config.secondaryHref}>{config.secondaryLabel}</NavLink></Button></div></div></div></div></section>;
}
