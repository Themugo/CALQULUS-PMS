      ...
      <div className="mx-auto max-w-[1480px] px-3 py-7 sm:px-4 lg:px-6">
        <div className="grid gap-6 lg:grid-cols-[1.2fr_2fr_1.25fr] lg:items-start">
          ...
          <div className="grid grid-cols-2 gap-6 sm:grid-cols-4">
            {config.shell.footer.columns.map((column) => <div key={column.id}><p className="text-[9px] font-bold tracking-[0.2em] text-white/90">{column.title}</p><div className="mt-2.5 flex flex-col gap-1.5 text-[10px] text-white/72">{column.items.map((item) => <FooterLink key={item.id} href={item.href} label={item.label}/>)}</div></div>)}
          </div>
          {config.shell.footer.showNewsletter ? <div><p className="text-[9px] font-bold tracking-[0.2em] text-white/90">{config.shell.footer.newsletterTitle}</p><div className="mt-2 flex overflow-hidden rounded-lg border border-white/10 bg-white"><input aria-label={config.shell.footer.newsletterTitle} placeholder={config.shell.footer.newsletterPlaceholder} className="min-w-0 flex-1 bg-transparent px-3 py-2 text-[10px] text-navy-deep outline-none placeholder:text-slate-400"/><button type="button" className="bg-success px-3 text-[10px] font-bold text-white">→</button></div><div className="mt-3 flex gap-1.5">{config.shell.footer.socials.filter((social) => social.enabled).map((social) => <FooterLink key={social.id} href={social.href} label={social.label}/>)}</div></div> : null}
        </div>
      </div>
