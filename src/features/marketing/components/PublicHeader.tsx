...
const navClass = "inline-flex min-h-10 items-center rounded-md px-2.5 py-1.5 text-[12px] font-semibold tracking-[0.01em] text-white/92 transition hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-success";
...
export function PublicHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-emerald-900/60 bg-[#004b3f] text-white shadow-[0_8px_24px_rgba(0,45,38,0.2)]">
      <div className="mx-auto flex h-[60px] max-w-[1480px] items-center justify-between gap-3 px-3 sm:px-4 lg:px-6">
        ...
        <div className="flex items-center gap-1.5">
          <a href="#quick-search" className="hidden h-9 w-9 items-center justify-center rounded-lg border border-white/15 bg-[#003A31] text-white transition hover:bg-white/10 sm:flex" aria-label={config.shell.header.searchLabel}><Search className="h-4 w-4"/></a>
          <Button asChild size="sm" className="min-h-9 rounded-lg bg-success px-4 text-[12px] font-bold text-white shadow-sm hover:bg-success/90"><Link to={PUBLIC_ROUTES.managerSignIn}>{config.shell.header.signInLabel}</Link></Button>
          <div className="hidden h-8 w-[58px] flex-col justify-center text-[8px] font-bold leading-3 tracking-[0.22em] text-white/92 xl:flex">{utilityWords.map((word) => <span key={word}>{word}</span>)}</div>
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="right" className="w-[min(100%,20rem)] border-white/10 bg-[#004b3f] text-white">
              ...
              <nav aria-label="Mobile" className="mt-6 flex flex-col gap-1">{navItems.map((item) => <HeaderLink key={item.id} href={item.href} label={item.label} onClick={() => setOpen(false)} dropdown={["solutions", "properties", "resources"].includes(item.id)} />)}<div className="mt-4 border-t border-white/15 pt-4"><HeaderLink href={"#quick-search"} label={config.shell.header.searchLabel} onClick={() => setOpen(false)}/><Button asChild className="mt-2 min-h-10 w-full bg-success text-white hover:bg-success/90 shadow-sm"><Link to={PUBLIC_ROUTES.managerSignIn} onClick={() => setOpen(false)}>{config.shell.header.signInLabel}</Link></Button></div></nav>
            </SheetContent>
          </Sheet>
        </div>
      </div>
