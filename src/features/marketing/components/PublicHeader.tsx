...
const navClass = "inline-flex min-h-10 items-center rounded-lg px-3 py-2 font-heading text-[13px] font-medium tracking-[0.01em] text-white/90 transition hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-light";
    ...
    <header className="sticky top-0 z-40 border-b border-[#0B2B7A] bg-[#123FB7] text-white shadow-[0_10px_28px_rgba(12,47,128,0.18)]">
      <div className="mx-auto flex h-[66px] max-w-[1480px] items-center justify-between gap-3 px-3 sm:px-4 lg:px-6">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Button asChild size="sm" className="min-h-9 rounded-lg bg-white px-4 font-heading text-[13px] font-semibold text-[#123FB7] shadow-sm hover:bg-white/90"><Link to={PUBLIC_ROUTES.managerSignIn}>{config.shell.header.signInLabel}</Link></Button>
          <div className="hidden h-9 w-[58px] flex-col justify-center font-heading text-[9px] font-bold leading-3.5 tracking-[0.22em] text-white/90 xl:flex">{utilityWords.map((word) => <span key={word}>{word}</span>)}</div>
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="right" className="w-[min(100%,20rem)] border-white/15 bg-[#123FB7] text-white">
              ...
              <nav aria-label="Mobile" className="mt-6 flex flex-col gap-1">{navItems.map((item) => <HeaderLink key={item.id} href={item.href} label={item.label} onClick={() => setOpen(false)} />)}<div className="mt-4 border-t border-white/15 pt-4"><HeaderLink href={config.rail.visible ? "#quick-search" : "#property-types"} label={config.shell.header.searchLabel} onClick={() => setOpen(false)}/><Button asChild className="mt-2 min-h-10 w-full bg-white font-heading text-[#123FB7] shadow-sm hover:bg-white/90"><Link to={PUBLIC_ROUTES.managerSignIn} onClick={() => setOpen(false)}>{config.shell.header.signInLabel}</Link></Button></div></nav>
            </SheetContent>
          </Sheet>
        </div>
      </div>
