...
const BankIntegrationSettings: React.FC = () => {
  ...
  const webhookUrl = (bankName: string, managerId: string, secret: string) =>
    `${SUPABASE_URL}/functions/v1/bank-webhook?manager_id=${managerId}&bank=${bankName}&secret=${secret}`;
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {integrations.map((intg: any) => {
                const wUrl = webhookUrl(intg.bank_name, user!.id, intg.webhook_secret);
                return (
                  <div key={intg.id} className="p-4 rounded-lg border border-border bg-muted/20">
                    ...
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground mb-1">Configure your bank to send push notifications to this URL:</p>
                      <div className="flex items-center gap-2">
