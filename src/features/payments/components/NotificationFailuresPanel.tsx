  ...
  const resolveMutation = useMutation({
    mutationFn: async ({ id, notes }: { id: string; notes?: string }) => {
      const { error } = await supabase
        ...
        .eq("id", id);
      if (error) throw error;
      // If the manager left notes, save them in a follow-up. We don't add
      // a `notes` column to notification_failures (kept slim) — instead,
      // notes go into activity_logs so the audit trail is unified.
      if (notes && notes.trim()) {
        const { data: { user } } = await supabase.auth.getUser();
        await supabase.from("activity_logs").insert({
          action:       "notification_failure_resolved",
          entity_type:  "notification_failure",
          entity_id:    id,
          entity_label: notes.slice(0, 200),
