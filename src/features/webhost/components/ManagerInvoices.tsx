...
const ManagerInvoices: React.FC<ManagerInvoicesProps> = ({ managers, invoices, isLoading, onRefresh }) => {
  ...
  const markAsPaid = useMutation({
    mutationFn: async (invoiceId: string) => {
      ...
      await supabase.rpc('reinstate_manager_on_payment' as any, { p_invoice_id: invoiceId })
  ...
  const handleRunEscalation = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('escalate_overdue_manager_invoices' as any);
      if (error) throw error;
      toast({
