...
const SystemLandlordManagement: React.FC = () => {
  ...
  const { data: payouts = [], isLoading: payoutsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return rows.map(r => ({
        ...r,
        property_name: propMap.get(r.property_id) ?? 'Property',
        landlord_email: profileMap.get(r.landlord_user_id) ?? 'Unknown',
      })) as PayoutRequest[];
    },
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-amber-200 bg-amber-50/50">
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-amber-900">System landlords only</p>
              <p className="text-xs text-amber-800 mt-0.5">
                You can only see landlords whose properties are not linked to any manager or agency.
      ...
      <div className="flex gap-2 border-b border-border">
        {[
          { key: 'landlords', label: `System Landlords (${landlords.length})` },
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between gap-3">
              <div>
                <CardTitle className="text-base">System landlords</CardTitle>
