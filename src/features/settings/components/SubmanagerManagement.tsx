  ...
  const handleCreateSubmanager = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: error.message || 'An error occurred while creating the submanager.',
        variant: 'destructive',
      });
    } finally {
      setIsCreating(false);
    ...
    try {
      ...
      if (agency?.id) {
        await (supabase.from('agency_members').upsert({
          agency_id: agency.id,
          manager_id: user.id,
          member_user_id: selectedSubmanager.submanager_user_id,
          role_in_agency: 'submanager',
          is_active: true,
        }, { onConflict: 'agency_id,member_user_id' }));
      }
