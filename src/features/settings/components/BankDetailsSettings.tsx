...
export const BankDetailsSettings = ({ propertyId, defaultScopeOnly }: BankDetailsSettingsProps = {}) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      let bankQuery: any = supabase
        .from('bank_details')
        .select('*')
        .eq('manager_id', user?.id)
        .order('is_default', { ascending: false });
      if (propertyId) bankQuery = bankQuery.eq('property_id', propertyId);
      else if (defaultScopeOnly) bankQuery = bankQuery.is('property_id', null);
      ...
      setAllUnits((unitsResult.data as unknown as Unit[]) || []);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast({ title: 'Error', description: error.message || 'Failed to delete bank details.', variant: 'destructive' });
    } finally {
      setDeleting(null);
    }
  };
  ...
  const updateAccount = (id: string, field: keyof BankDetails, value: any) => {
