...
interface Tenant { id: string; name: string; email: string; phone: string | null; unit: string | null; property: string | null; }
...
const BroadcastCenter: React.FC = () => {
  ...
  const { data: tenants = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('tenants').select('id, name, email, phone, unit, property').eq('manager_id', user!.id).eq('status', 'active').order('name');
  ...
  const audienceTenants = React.useMemo(() => {
    ...
    if (audienceType === 'property_tenants' && selectedPropertyId) {
      result = tenants.filter(t => t.property === properties.find(p => p.id === selectedPropertyId)?.name);
    ...
    return result;
  }, [tenants, audienceType, selectedPropertyId, selectedTenantId, tenantSearch, properties]);
  ...
  const sendBroadcast = useMutation({
    mutationFn: async () => {
      ...
      const notifyFn = async (tenant: Tenant) => {
        ...
        if (sendApp) {
          await supabase.from('in_app_notifications').insert({
            manager_id:   user!.id,
            user_id:      tenant.id, // will need to look up auth user_id
        ...
        const notify = async (fn: string, payload: Record<string, unknown>) => {
          await supabase.functions.invoke(fn, { body: payload }).catch(() => {});
        };
        ...
        if (sendEmail && tenant.email) {
          ...
          await notify('send-invoice-email', {
            to: tenant.email, tenantName: tenant.name,
      ...
      // Send to all recipients (chunked to avoid timeout)
      await Promise.allSettled(audienceTenants.map(notifyFn));
