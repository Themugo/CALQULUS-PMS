  ...
  const filtered = providers.filter((p: any) => {
    if (search) {
      ...
      if (!p.business_name?.toLowerCase().includes(q) &&
          ...
          !p.provider_services?.some((s: any) => s.service_categories?.name?.toLowerCase().includes(q))) {
        return false;
      }
    }
    if (selectedCategory !== 'all') {
      if (!p.provider_services?.some((s: any) => s.category_key === selectedCategory)) return false;
    }
    if (selectedGroup !== 'all') {
      if (!p.provider_services?.some((s: any) => s.service_categories?.group_name === selectedGroup)) return false;
    }
    return true;
  });

  const groups = [...new Set(categories.map((c: any) => c.group_name))];
  const selectedProvider = providers.find((p: any) => p.id === profileOpen);
          ...
          {selectedGroup !== 'all' && (
            <Select value={selectedCategory} onValueChange={setSelectedCategory}>
              ...
              <SelectContent>
                ...
                {categories.filter((c: any) => c.group_name === selectedGroup).map((c: any) => (
...
const ProviderProfile: React.FC<{ provider: any; onSelect?: () => void }> = ({ provider, onSelect }) => {
  const services = provider.provider_services || [];
