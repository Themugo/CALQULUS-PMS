...
const ServicesPage: React.FC = () => {
  const { user, isTenant, isManager } = useAuth();
  ...
  const { data: assignedJobs = [], isLoading: jobsLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .eq('assigned_provider_id' as any, myProvider!.id)
  ...
  return (
    <Layout
      title="Services"
      subtitle="Find service providers or manage your own provider profile"
    >
      <Tabs defaultValue="marketplace">
        <TabsList className="flex-wrap h-auto gap-1 p-1 mb-6">
          ...
          <TabsTrigger value="my-profile" className="gap-1.5">
            ...
            My provider profile
            {myProvider && <Badge variant="outline" className="ml-1 text-xs bg-green-100 text-green-800 border-green-300">Active</Badge>}
          </TabsTrigger>
          {myProvider && (
                          ...
                          <div className="shrink-0">
                              ...
                              <Button size="sm" className="h-8 text-xs gap-1 bg-amber-600 hover:bg-amber-700 text-white"
                                onClick={async () => {
                                  await supabase.from('maintenance_requests')
                                    .update({ status: 'in_progress', provider_started_at: new Date().toISOString() } as any)
                                    .eq('id', job.id);
                                }}>
                                Start job
                              </Button>
                            ...
                            {job.status === 'in_progress' && (
                              <Button size="sm" className="h-8 text-xs gap-1 bg-green-600 hover:bg-green-700 text-white"
                                onClick={async () => {
                                  await supabase.from('maintenance_requests')
                                    .update({ status: 'completed', provider_completed_at: new Date().toISOString(), completion_date: new Date().toISOString().slice(0,10) } as any)
                                    .eq('id', job.id);
                                }}>
