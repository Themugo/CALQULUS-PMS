...
export default function UnitUtilityMeters({ unitId, propertyId, unitLabel }: UnitUtilityMetersProps) {
  ...
  const addMeter = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.rpc('save_unit_utility_meter_atomic', {
        p_unit_id: unitId, p_property_id: propertyId, p_utility_type: form.utility_type,
  ...
  const recordReading = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.rpc('update_unit_utility_meter_reading_atomic', { p_meter_id: readingMeter.id, p_reading: reading });
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey });
  ...
  const toggleActive = useMutation({
    mutationFn: async (meter: UtilityMeter) => {
      const { error } = await supabase.rpc('set_unit_utility_meter_active_atomic', { p_meter_id: meter.id, p_is_active: !meter.is_active });
      if (error) throw error;
    },
  ...
  const removeMeter = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.rpc('delete_unit_utility_meter_atomic', { p_meter_id: id });
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey });
