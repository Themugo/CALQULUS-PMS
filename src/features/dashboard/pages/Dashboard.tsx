...
const Dashboard = () => {
  ...
  // Check if this manager has completed onboarding (has an agency)
  const { data: hasAgency, isLoading: agencyLoading } = useQuery({
    queryKey: ['manager-has-agency', user?.id],
    queryFn: async () => {
      const { data } = await supabase.from('agencies').select('id').eq('manager_id', user!.id).maybeSingle();
      return !!data;
    },
    enabled: !!user?.id,
  });
  ...
  const showOnboarding = !agencyLoading && !hasAgency && !onboardingDismissed;
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      const [
        ...
        supabase.from("tenants").select("id", { count: "exact", head: true }).eq("manager_id", user!.id).eq("status", "inactive"),
        supabase.from("tenants").select("id", { count: "exact", head: true }).eq("manager_id", user!.id).gte("created_at", startOfMonth),
        ...
        supabase.from("invoices").select("amount").eq("manager_id", user!.id).eq("status", "paid").gte("paid_date", startOfMonth),
        supabase.from("invoices").select("amount").eq("manager_id", user!.id).eq("status", "paid").gte("paid_date", startOfLastMonth).lte("paid_date", endOfLastMonth),
        supabase.from("properties").select("units, occupied").eq("manager_id", user!.id),
      ...
      const revenueMTD = revenueMTDResult.data?.reduce((sum, inv) => sum + Number(inv.amount), 0) || 0;
      const revenueLastMonth = revenueLastMonthResult.data?.reduce((sum, inv) => sum + Number(inv.amount), 0) || 0;
      const revenueChange = revenueLastMonth > 0 ? Math.round(((revenueMTD - revenueLastMonth) / revenueLastMonth) * 100) : 0;

      const totalUnits = propertiesResult.data?.reduce((sum, p) => sum + p.units, 0) || 0;
      const totalOccupied = propertiesResult.data?.reduce((sum, p) => sum + p.occupied, 0) || 0;
      ...
      setStats({
        ...
        arrearsTotal: overdueAmountResult.data?.reduce((s, i) => s + Number(i.balance_due ?? 0), 0) || 0,
      });
    } catch {
    } finally {
      setLoading(false);
    }
  }, [user, setStats, setLoading]);
    ...
    >
      ...
      {showOnboarding && (
        <ManagerOnboarding onComplete={() => {
          ...
          queryClient.invalidateQueries({ queryKey: ['manager-has-agency'] });
        }} />
      )}
      ...
      {user?.email?.includes('@rentflow.ink') && (
        <div className="mb-4 rounded-xl border border-amber-300/60 bg-amber-50/80 px-4 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-sm text-amber-800">
            ...
            <span><strong>Demo mode:</strong> You're logged in as a demo account. All data is sample data and can be reset.</span>
          </div>
          <a href="/demo" className="shrink-0 text-xs font-semibold text-amber-700 underline hover:text-amber-900">
            Demo panel →
          </a>
        </div>
      )}
