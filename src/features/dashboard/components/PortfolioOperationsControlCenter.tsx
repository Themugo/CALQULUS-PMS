...
export function PortfolioOperationsControlCenter({ stats, loading = false }: Props) {
  ...
  const items = [
    { id: "arrears", label: "Collections", count: stats.overdueInvoices, detail: `${formatCurrency(stats.arrearsTotal)} overdue`, href: "/billing?filter=overdue", icon: CreditCard, tone: stats.overdueInvoices ? "danger" : "clear" },
    { id: "maintenance", label: "Maintenance", count: stats.openMaintenanceCount, detail: `${stats.urgentMaintenanceCount} urgent`, href: "/maintenance", icon: Wrench, tone: stats.urgentMaintenanceCount ? "danger" : stats.openMaintenanceCount ? "warning" : "clear" },
    { id: "leases", label: "Lease renewals", count: stats.expiringLeases, detail: "Expiring within 30 days", href: "/leases", icon: FileClock, tone: stats.expiringLeases ? "warning" : "clear" },
    { id: "vacancy", label: "Vacancies", count: stats.vacantUnits, detail: `${stats.totalUnits} total units`, href: "/properties", icon: Home, tone: stats.vacantUnits ? "info" : "clear" },
    { id: "payments", label: "Payment exceptions", count: paymentCount, detail: "Reconciliation or recovery needed", href: "/billing", icon: AlertTriangle, tone: paymentCount ? "danger" : "clear" },
  ] as const;
