...
const statusStyles: Record<string, string> = {
  ...
  pending: "bg-gradient-to-br from-warning/10 to-warning/5 text-warning border-warning/30",
      ...
      {tenants.length === 0 ? (
        ...
        <div className="space-y-2">
          {tenants.map((tenant, index) => (
            ...
            >
              <Avatar className="h-12 w-12 sm:h-14 sm:w-14 border-2 border-border/50 group-hover:border-warning/30 transition-all duration-300">
                <AvatarImage src={signedUrls[tenant.id]} alt={tenant.name} />
                <AvatarFallback className="bg-gradient-to-br from-warning/20 to-amber-400/8 text-warning text-sm sm:text-base font-semibold">
