...
export const generatePropertyStatementData = async (
  ...
  const waterReadings = (waterRes.data || []) as any[];
  const deductions = (deductionsRes.data || []) as any[];
  const amenityCharges = (amenityRes.data || []) as any[];
  const leases = (leasesRes.data || []) as any[];

  // Filter invoices for tenants in this property
  const tenantIds = tenants.map((t: any) => t.id);
  const propertyInvoices = allInvoices.filter((inv: any) => tenantIds.includes(inv.tenant_id));

  let totalRentCollected = 0;
  const totalDeposits = 0;
  let totalWaterFees = 0;

  const unitRows: UnitRow[] = units.map((unit: any) => {
    const tenant = tenants.find((t: any) => t.unit_id === unit.id && t.status === "active");
    const lease = leases.find((l: any) => l.unit_id === unit.id);
    ...
    const tenantInvoices = tenant
      ? propertyInvoices.filter((inv: any) => inv.tenant_id === tenant.id && !inv.description?.toLowerCase().includes("water"))
      : [];
    ...
    const unitWater = waterReadings.filter((r: any) => r.unit_id === unit.id);
    const waterBill = unitWater.reduce((s: number, r: any) => s + Number(r.total_amount || 0), 0);
    const waterPaid = unitWater.filter((r: any) => r.status === "paid").reduce((s: number, r: any) => s + Number(r.total_amount || 0), 0);

    // Amenity charges for this unit (or property-wide with no unit_id)
    const unitAmenities = amenityCharges.filter((a: any) => a.unit_id === unit.id || !a.unit_id);
    const garbage = unitAmenities
      .filter((a: any) => a.charge_type === "garbage")
    ...
    const paidInvoice = tenantInvoices.find((inv: any) => inv.status === "paid");
  ...
  const deductionRows: DeductionRow[] = deductions.map((d: any) => ({
...
export const generatePropertyStatementPDF = async (
  ...
  const tableBody: any[] = data.units.map((u) => [
    u.unit_number,
    n(u.rent_payable),
    u.tenant_name,
