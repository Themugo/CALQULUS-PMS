...
const PropertyDetail = () => {
  ...
  const fetchPropertyData = useCallback(async () => {
    ...
    const [agencyRes, tenantsRes, leasesRes, allTenantsRes] = await Promise.all([
      propertyData.agency_id
        ? supabase.from("agencies").select("id, name").eq("id", propertyData.agency_id).maybeSingle()
  ...
  const handleAssignTenant = async () => {
    ...
    // Look up or create unit record
    let unitId: string | null = null;
    const { data: existingUnit } = await supabase
      .from("units")
      .select("id")
      .eq("property_id", property.id)
      .eq("unit_number", assignUnit.trim())
      .maybeSingle();
    ...
    if (existingUnit) {
      ...
      const { data: newUnit } = await supabase
        .from("units")
        .insert({ property_id: property.id, unit_number: assignUnit.trim(), status: "occupied" })
        .select("id")
        .single();
      ...
      <Tabs defaultValue={initialTab} className="w-full">
        ...
        <TabsContent value="units">
          <UnitManagement 
            ...
            houseLabelPrefix={(property as any).house_label_prefix || ""}
