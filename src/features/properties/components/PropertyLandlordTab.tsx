...
const PropertyLandlordTab: React.FC<PropertyLandlordTabProps> = ({ propertyId }) => {
  ...
  const { data: landlordLink, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select('id, landlord_user_id, revenue_share_pct, assigned_at')
      ...
      const { data: profile } = await supabase
        ...
        .eq('id', (data as any).landlord_user_id)
        .maybeSingle();
  ...
  const inviteLandlord = useMutation({
    mutationFn: async () => {
      ...
      const { data: existingRoles } = await supabase
        .from('user_roles')
        .select('user_id')
        .eq('role', 'landlord' as any);

      if (existingRoles && existingRoles.length > 0) {
        const userIds = existingRoles.map((r: any) => r.user_id);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Mark Paid dialog */}
      {payDialogOpen && (() => {
        const payout = payouts.find((p: any) => p.id === payDialogOpen);
