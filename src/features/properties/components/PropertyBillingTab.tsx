...
export function PropertyBillingTab({ propertyId, propertyName }: PropertyBillingTabProps) {
  ...
  const fetchInvoices = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("invoices")
      .select(`*, leases (property, unit), tenants (id, name, email, phone, photo_url)`)
  ...
  const fetchLeases = useCallback(async () => {
    const { data } = await supabase
      .from("leases")
      .select(`id, property, unit, monthly_rent, tenant_id, tenants (id, name, email, photo_url)`)
      .eq("property_id", propertyId)
      .eq("status", "active")
      .order("unit");
  ...
  const handleMarkAsPaid = async (invoiceId: string) => {
    const { error } = await supabase.from("invoices").update({ status: "paid" as any, paid_date: new Date().toISOString().split("T")[0] }).eq("id", invoiceId);
    if (!error) {
  ...
  const handleVoidInvoice = async (invoiceId: string) => {
    ...
    const { error } = await supabase.from("invoices").update({ status: "cancelled" as any }).eq("id", invoiceId);
    if (!error) {
                          ...
                          <TableCell className="text-right">
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                <DropdownMenuItem onClick={() => downloadInvoicePDF(invoice as any)}>
                      ...
                      <TableRow key={inv.id}>
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-1">
                            <Button size="sm" variant="ghost" onClick={() => downloadReceiptPDF(inv as any)}>
