...
describe("CALQULUS design tokens", () => {
  ...
  it("uses interactive blue as primary, not gold or cyan", () => {
    expect(CALQULUS_COLOR.primary).toBe("#356FE5");
    expect(CALQULUS_COLOR.primaryHover).toBe("#285FCC");
    expect(CALQULUS_COLOR.primaryActive).toBe("#214FAE");
  ...
  it("defines the full semantic palette", () => {
    ...
    expect(CALQULUS_COLOR.glow).toBe("#356FE5");
  ...
  it("exposes portal accents without replacing the desk system", () => {
    ...
    expect(CALQULUS_PORTAL_ACCENT.agency.hex).toBe("#A66A16");
...
describe("index.css Tailwind v4 production safety", () => {
  ...
  it("keeps CSS variables in lockstep with the TypeScript palette", async () => {
    ...
    expect(css).toContain(`[data-portal="landlord"] { --portal-accent: var(--calqulus-success)`);
    expect(css).toContain(`[data-portal="agency"] { --portal-accent: var(--calqulus-warning)`);
