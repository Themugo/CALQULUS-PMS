...
describe("CALQULUS design tokens", () => {
  ...
  it("defines the full semantic palette", () => {
    ...
    expect(CALQULUS_COLOR.glow).toBe(CALQULUS_COLOR.brandBlue);
...
describe("index.css Tailwind v4 production safety", () => {
  ...
  it("keeps CSS variables in lockstep with the TypeScript palette", async () => {
    ...
    expect(css).toContain(`--calqulus-brand-blue: ${CALQULUS_COLOR.brandBlue}`);
    ...
    expect(css).toContain(`[data-portal="agency"] { --portal-accent: var(--calqulus-agency)`);
