...
## Brilliant-Navy Premium Landing Redesign (2026-08-27)
- **New visual identity replaces deep navy**: landing public homepage now uses the **brilliant-navy** palette (#123F8C / #0B2F6B / light #EAF2FF, accent #2F6FED, cyan #16B8C4, bg #F7F9FC, surface white, text #10233F/#5E6F86, border #DCE5F0). No black backgrounds; navy is chrome-only; white/light surfaces carry the page. This replaces the older `HOMEPAGE_ROLE_ACCENTS`/navy-deep approach described above.
- **Centralized design tokens**: `src/features/marketing/theme/landingTheme.ts` (`LANDING_THEME`, `landingThemeToCssVars`, `isLight`) → writes CSS custom properties `--calqulus-landing-*` which the `@theme` block in `src/index.css` maps to `--color-landing-*` Tailwind v4 utilities (`text-landing-textsecondary`, `bg-landing-primary`, `border-landing-border`, …). Components consume tokens only — never hard-coded hexes in JSX.
- **Config-driven content model**: `src/features/marketing/landing/landingContent.ts` defines `LandingPageConfig` = `{ theme, brand, header, hero, dashboard, trust, capabilities[], roles[], propertyTypes[], metrics[], finalCta, footer, sections[] }`. Defaults in `defaultLandingConfig.ts`. `sections[]` controls display order (future CMS). Every string, icon (via safe string→Lucide resolver `landingIcon.ts`, registry `LANDING_ICON_NAMES`), accent colour, image and CTA label/href is editable. No content is hard-coded in presentation components.
- **Editability abstraction**: `contentService/` (`createLandingContentAdapter`, `canEditLandingSection`, `LANDING_PERMISSIONS`, `LANDING_EDITOR_ROLES`) — a typed adapter/service boundary for future Webhost/Admin control. No fabricated DB persistence; frontend permissions are UX-only, backend stays authoritative.
- **New components** under `src/features/marketing/landing/components/`: `PremiumHeader` (sticky white header, mobile Sheet), `PremiumHero` (two-column: headline+CTAs+3 trust points / `DashboardPreview`), `DashboardPreview` (metric cards, 7-week collections chart, portfolio performance, "Needs attention" — illustrative data clearly marked), `TrustStrip`, `CapabilityGrid` (6 cards), `RoleSolutions` (6 role cards with role colours: Manager navy/Landlord teal/Agency slate-blue/Tenant emerald/Admin indigo/Webhost cyan), `PropertyTypeStrip` (Residential/Commercial/Office photos), `PlatformMetrics` (illustrative indicators, footnote "Values marked · sample …"), `FinalCta`, `PremiumFooter` (Product/Solutions/Resources/Company/Legal, all live routes, external mailto handled).
- **Assembly**: `LandingPage.tsx` provides `LandingSections` (config-ordered sections wired with nav anchor ids `#capabilities`, `#roles`, `#property-types`, `#resources`, `#company`) + `LandingPageInner`. `MarketingWebsite.tsx` renders `/` from `defaultLandingConfig`. `/pricing` keeps `PublicPricing` tiers (KES 400/600/custom per property/month) under the shared shell; Enterprise "Talk to us" now routes to `mailto:CONTACT_EMAIL` (was dead `#contact`).
- **Dead code removed**: old visual components deleted (`ExecutiveHero`, `OperationalWorkflow`, `PlatformOverview`, `PortalExperiences`, `CompactCta`, `TrustSection`, `PropertyTypeSlider`, `ArchitecturalSurface`, `ProductPreview`); `propertyImages.ts` now self-contains its `PropertyVisualSlot` type. `PublicShell`/`PublicHeader`/`PublicFooter`/`PublicPricing` kept (Legal page uses `PublicShell`).
- **Tests**: `landingConfig.test.ts` (token→CSS var mapping, icon resolver safety, config model), `publicConfig.test.ts`, `publicLanding.test.tsx` (portal routes, single h1, illustrative metrics footnote, lovely Navy CTA). Pitch-once Pricing h1 via `SectionHeading level={1}`. Gateway: **1211 passed / 1 skipped**, lint 0 errors (11 pre-existing warnings), `tsc --noEmit` clean, `npm run build` clean. Playwright-core + system chromium verified 0 px horizontal overflow at 1440/1280/1024/768/430/390/360, single h1 on `/` and `/pricing`, mobile menu ≤768px, skip-link + accessible images present.
## Landing Content Editor (Webhost/Admin CMS, 2026-08-27)
- **Wired `contentService` to a live Supabase provider**: `createSupabaseLandingContentProvider()` implements `getConfig()` (reads the single published `landing_page_content` row, merges over `defaultLandingConfig`) and `saveSection()` (calls the authoritative `upsert_landing_section` RPC). The page singleton `landingContent` is now the Supabase provider. When Supabase env is absent, the noop client returns `{ error }` so reads fall back to defaults and writes fail gracefully — no fabricated persistence ever.
- **Backend migration `20260827000001_landing_content.sql`** (must be applied to live DB): `landing_page_content` table (single `scope='landing'` row, JSONB config, published flag, audit cols). RLS: public/anonymous SELECT when `published`; webhost platform admins (`user_roles.role='webhost'` + `platform_admins` not suspended) can INSERT; full editors (owner/business/can_manage_platform_settings) can UPDATE/DELETE. `upsert_landing_section(section, payload)` is SECURITY DEFINER — enforces the section whitelist server-side (plain `admin` may only edit hero/capabilities/roles/metrics), merges into the row, idempotent.
- **Editor UI**: `/webhost/landing` → `src/features/webhost/pages/AdminLandingContent.tsx` (lazy route, `protected: true`), nav item "Landing Content" in WebhostLayout Account group. Resolves the editor role from `platform_admin.admin_type`: owner/business/can_manage_platform_settings → full; plain admin → scoped (hero/capabilities/roles/metrics only). Tabbed per-section form (header/hero/dashboard/trust/capabilities/roles/propertyTypes/metrics/finalCta/footer/brand/theme), live full-page preview reusing `LandingPageInner`, per-section Save through `saveSection`. Frontend permissions are UX-only — the RPC + RLS are authoritative.
- **Role model corrected**: `LANDING_EDITOR_ROLES` = `["webhost","admin"]` (there is no `platform_admin` in `user_roles`; the tier lives on `platform_admins.admin_type`). `resolveLandingEditorRole(role, adminType, canManagePlatformSettings)` maps the webhost identity to the editor scope.
- **Tests**: `landingContentEditor.test.ts` (route registration, nav link, provider/Supabase wiring, permission resolution) + `landingConfig.test.ts` extended. Full suite **1225 passed / 1 skipped**; lint 0 errors (11 pre-existing warnings); build clean.

## Landing CMS refinements (2026-08-27)
- **Seed published row**: `supabase/seed/landing_content.sql` — idempotent; inserts one published `scope='landing'` row with `'{}'::jsonb` config if none exists, so the editor always merges into a published source of truth (empty = packaged defaults). Run it (`supabase db reset` or SQL Editor) before editing content live.
- **Asset-store uploads**: `contentService.uploadLandingAsset(file, folder?)` uploads to the **`landing-images`** storage bucket (cache-control 1yr, sanitized path) and returns the public URL. Service falls back to `{ ok:false }` (never pretends). Editor gained an `AssetField` (paste URL or upload). The bucket is provisioned by migration `20260827000002_landing_assets_bucket.sql` (public read, webhost-platform-admin write via `is_webhost_platform_admin()`); upload fails gracefully if the bucket is absent.
- **Apply-live bundle**: `supabase/sql/apply-live-landing-cms.sql` concatenates the landing CMS table+RLS+RPC migration, the published seed, and the bucket migration into one paste-ready script (all idempotent) for the live SQL Editor.
- **audit:prod gate fix**: the app's own public Supabase host is exempt in `scripts/audit-production.mjs` only for `src/test/adminDesk.test.ts` (legit backend-project assertion). It is still forbidden everywhere else in `src`; the CSP already allows `https://*.supabase.co`.
- **Section ordering**: `sections` added to `LandingSectionKey` and to `LANDING_PERMISSIONS.webhost` (full-editor only; scoped `admin` denied, RPC whitelist agrees). Editor gained an **Order** tab (`OrderEditor`: move up/down, hide/remove) that reorders `config.sections` — the renderer already consumes `sections` at runtime, so live order changes apply.
- **Audit trail**: editor's `handleSave` calls `logLandingEdit` → `log_activity` (`p_action='content_edit'`, `p_entity_type='landing_page_content'`, metadata `{section, editorRole}`) after each successful save; non-blocking so audit failures never break saves.
- **Nav gating**: `WebhostLayout.canSee` hides the Account-group **Landing Content** item unless `resolveLandingEditorRole(userRole.role, platformAdminInfo.admin_type, platformAdminInfo.can_manage_platform_settings) !== null` (full webhost or scoped admin). Explicit branch parallels the unattached-tenants gate.
- Gates: tsc clean · lint 0 errors (11 pre-existing) · **1225 passed / 1 skipped** · build clean (precache 773.80 KiB).
