...
- **Verified, not redesigned**: the existing certification suites were run and extended. `e2e/responsive-certification.spec.ts` now covers all 8 required widths (1920/1440/1280/1024/768/480/390/360) across 11 design-preview portal screens + 5 login pages (zero horizontal overflow), dialogs inset at 360px, and **tenant touch targets ≥44px** asserted at ≤480px. `e2e/a11y.spec.ts` runs axe (wcag2a/aa, critical+serious = 0) on homepage/design-preview/all 5 logins + skip-link keyboard focus + dialog labelling + table column headers.
- **Fixes (all axe-flagged colour contrast on the public homepage)**: `PublicFooter` `text-white/68`→`/75` (tagline) and `text-white/48`→`/75` (legal bar) — footer bg is mid-navy `#31577E`, not deep navy. `PortalExperiences` "View portal" links now mix accent 50% (was 72%) with navy-deep → ≥4.5:1 on the tinted card backgrounds for every portal accent. `DesignPreview` swatch list keyed by `label` (was `hex` — navy-deep/navy-mid share `#31577E`, duplicate key warning).
...
- Results: E2E chromium 91 passed / 116 skipped (credential-gated); unit 1148 passed / 1 skipped; lint 0 errors; typecheck + build clean. Note: `resendVerification.test.tsx` "does not start a cooldown when the send fails" is flaky under full-suite parallelism (passes in isolation, passes on re-run) — not Phase 10 related.
...
- **Duplication removed**: 14 feature screens rendered raw `py-12 text-center` empty blocks. Converted to shared `EmptyState` (Services, Marketplace, VacationNotices, PropertyCollectionStatement, ActivityLog, ManagerManagement, SystemLandlordManagement, LandlordBilling, RentCollectionSummary, LandlordLinksManager, TenantInbox, TenantPortableHistory, ManagerPaymentHistory, ManagerPlatformBilling). TenantInbox error path → shared `ErrorState` with real `refetch` retry. Markup-only; no query/API changes.
- **Locked by test**: `src/test/componentAuditPhase9.test.ts` (14) — button hierarchy, Field anatomy, statusBadge tones + badge variants, table chrome, converted screens, single icon family, no second library (MUI/AntD/Chakra/react-icons/iconify banned). Suite: 1148 passed / 1 skipped; lint 0 errors (11 pre-existing warnings); typecheck clean; build OK.
...
- **accept-admin-invitation**: seeds `platform_admins` server-side (tier from invitation, never client; `is_immutable: false`; business → can_create_admins/billing/platform_settings) + aligns `admin_permissions` (`business` → `admin` level + billing/webhost creation; `admin` → `limited_admin`). platform_admins upsert is safe under service_role (no FORCE RLS on the table).
- **UI**: `/webhost/invite` card now shows "Operator access: Business/Admin operator" (`adminTierLabel` helper in `adminInvitation.ts`).
- **Secrets hygiene verified**: `lib/secrets.ts` (`isSecretKey`/`maskSecrets`/`stringifyMasked`) + `getNonSecretConfig` (infrastructure.ts) — no credential-shaped key or value reaches any screen during onboarding or ops views.
...
- Tests: `src/test/webhostOperatorOnboarding.test.ts` (20) — no public signup route, no client role/tier params, owner-ungrantable, platform_admins server-side seeding, secrets masking (10 credential key shapes + getNonSecretConfig), session expiry handling, unauthorized access (manager config has no webhost components). Suite: 1049 passed / 1 skipped.
...
- **Audited first — no parallel system added.** Live flow stays: `tenant_invitations` + `validate_invitation_token` RPC + `send-tenant-invitation` + `create-tenant-account` + `/tenant/invitation` → `TenantAuth.tsx`. Legacy `tenant_invites`/`accept-tenant-invite` edge function is unused by the app — left untouched.
- **Server is now authoritative** (`create-tenant-account` `invitationToken` path): token is the credential (not caller role — fixes the 401/403 invited tenants hit post-signUp). Property/unit/manager/rent resolved from the invitation row server-side; client-supplied IDs ignored. Auth user email must match the invited email (`invitation_email_mismatch`, 403). Expired → 410 `invitation_expired`; used → 410 `invitation_used` unless the same user already claimed (idempotent summary via `user_roles.tenant_id` — refresh/back-safe). Mark-used is server-side, guarded by `status='pending'`.
- **Migration `20260824000001_tenant_invitation_phase7.sql`** (mirrored into `supabase/sql/apply-live-p1-rpcs.sql`): `validate_invitation_token` now returns `inviter_name` (company_settings.company_name → profiles.full_name → 'Your property manager') + `phone`; new PII-free `invitation_token_state(token)` → pending/expired/used/invalid. Both granted to anon+authenticated. **Must be applied to live DB before the new UI works.**
- **TenantAuth invitation flow**: "You're invited to access your property account." + Property/Unit/Invited-by card (org name only — manager email/phone no longer fetched or shown to token holders). Email read-only (pre-associated). Distinct expired/used/invalid screens via `invitation_token_state`. Post-signup: session → confirmation screen (Property/Unit/rent/deposit from server summary) + "Enter Tenant Portal"; no session → verification screen (previously dead code, now wired). Self-registration (accounting mode) unchanged.
- Pure helpers in `src/features/auth/lib/tenantInvitation.ts` (state mapping, email match, summary builders — lease figures only when set, never fabricated).
- Tests: `src/test/tenantInvitation.test.ts` (21) — valid/expired/used/invalid tokens, wrong-email binding, refresh+back stability, summary builders, plus static security invariants on the edge function + migration. Suite: 1001 passed / 1 skipped.
...
## Frontend Refinement Phase 6 — Agency Portal (2026-08-24)
- **Identity**: navy + white with amber sparingly (portal accent). Money is never green; amber reserved for the attention status chip + thin performance bars + chart "Outstanding" line + small icon accents (`text-[var(--portal-accent)]`).
- **Pure model** `src/features/agency/lib/agencyPortfolio.ts`: `agencyClientStatus` (pending → invitation pending; outstanding>0 → attention; else active), `agencyClientStatusChipClass` (amber only for attention; active/pending neutral — **never success green**), `agencyCollectionRate(collected, outstanding)` (0 when nothing billed), `buildAgencyAttentionItems` (arrears → leases → unlinked), `AGENCY_TREND_COLORS` (navy-mid collected / warning amber outstanding), `AGENCY_CLIENT_TABS` order (overview → portfolio → financial → maintenance → activity → documents).
- **Client detail**: new route `/agency/clients/:id` → `AgencyClientDetail.tsx` (status chip + email, six tabs; pending-invitation synthesised IDs `pending:<propertyId>` resolve to the single building). Documents tab queries `landlord_documents` by `manager_id` + `landlord_user_id` (RLS `manager_manages_landlord_documents` — backend permissions unchanged). Activity tab reuses `ManagerActivityLog` (scoped `manager_id = auth.uid()`). Maintenance tab counts open requests across the client's buildings. Unknown client ID → "not linked to your agency" guard.
...
- **Portfolio**: totals strip (Properties, Units, Occupancy, Collected this month, Outstanding [destructive when >0], Clients) + performance-bar column (amber portal-accent width vs max collected) — table-based, no card overload.
- **Dashboard**: stat strip (Clients, Properties, Units, Occupancy, Collections) → Needs attention (arrears/leases/unlinked + open maintenance count) → Portfolio activity chart (navy collected vs amber outstanding + legend) → Client portfolio performance → Property performance (top 5) → Recent activity.
- Tests: `src/test/agencyPortalPhase6.test.ts` (13) — status mapping, amber-only chip, collection rate, attention ordering, tab lock, route registration, no-nested-manager-Layout invariant, no `text-success`/`text-green` money. Suite: 1104 passed / 1 skipped; one pre-existing flake (`resendVerification` cooldown race) reproduces 1-in-5 on clean main — unrelated.
- Previews captured: before (dashboard/clients/portfolio) and after (dashboard with property performance + recent activity, portfolio, client detail guard) on dev server port 12000 (empty states — no Supabase env in sandbox).
