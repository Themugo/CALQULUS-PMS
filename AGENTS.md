...
- **State:** package.json already pinned to very recent majors (react 19, vite 8, vitest 4, eslint 10, typescript-eslint 8.67, tailwindcss 4, recharts 3, react-router 7, react-day-picker 10). `npm outdated` showed only `typescript` (6.0.3 -> 7.0.2) as genuinely outdated.
- **TypeScript 7.0.2 upgrade REVERTED:** `tsc`/`vite build`/`vitest` all pass on TS 7, but `npm run lint` fails with hard error `typescript-eslint does not support TS 7.0` (typescript-eslint peer range is `>=4.8.4 <6.1.0`; tracked in typescript-eslint #10940/#12518). Microsoft's guidance: wait for TS 7.1's stable programmatic API. Do NOT bump `typescript` past 6.x until typescript-eslint ships TS-7 support. Kept at `^6.0.3`.
- **Pruned:** removed `@types/dompurify` (dompurify@3 ships its own `.d.ts`; package unreferenced). Added `overrides: { "uuid": "^11.1.1" }` to fix the moderate `uuid < 11.1.1` advisory pulled in transitively by `@capacitor/cli -> xcode` (uuid is NOT used in app source; dev-only transitive). `npm audit` now reports **0 vulnerabilities** (was 3 moderate).
- **False-positive "unused" deps (do NOT remove):** `@capacitor/android`, `@capacitor/ios` (native platform packages for `npx cap add`); `@tailwindcss/postcss`, `postcss`, `tailwindcss` (used by `postcss.config.js` + Tailwind v4); `vite-plugin-pwa` (used in `vite.config.ts`). depcheck/knip flag these but they are required build/native deps.
- **Orphaned source (not deps, not touched):** `src/lib/rate-limit/middleware.ts` imports undeclared `@upstash/redis` (resolves transitively via `@upstash/ratelimit`); `src/lib/mobile/app-lifecycle.ts` imports undeclared `@capacitor/app` (NOT installed). Neither file is imported anywhere, so they sit outside the tsc/vite graph (tsc passes despite the missing module). knip reports 331 unused files total (analytics/compliance/accounting modules). Out of scope for dependency work; do not bulk-delete without an explicit ask.
- **Verified after changes:** `tsc --noEmit` clean, `eslint .` 0 errors (19 pre-existing warnings), `vitest run` 690/690 pass (38 files), `vite build` success, `npm audit` 0 vulnerabilities, dev server responds HTTP 200 on :3000.

## Landlord Portal Optimization (2026-08-14)
- **Scope:** Authenticated landlord portal ONLY (`/landlord/dashboard` + `src/features/landlord/*`). Manager/Agency/Tenant/Super Admin/landing untouched. No backend/DB/RLS/auth changes.
- **Visual identity:** Added `.landlord-portal` scoped CSS-variable overrides in `src/index.css` on top of the shared light executive design system. Primary accent `#059669` (emerald-600), secondary `#0D9488` (teal-600), accent-soft `#ECFDF5`, page bg `#F4F7FB`, surfaces `#FFFFFF`, text `#0F172A`/`#334155`/`#64748B`, border `#DCE3ED`. No dark/black backgrounds. These are scoped overrides of existing tokens so white-label customers can re-skin by overriding the same vars — no new branding architecture. Reuses `enterprise-card`, `page-title`, `section-title`, `metric-value`, `card-title-exec`, `supporting-text`, `meta-text`, `status-badge` utilities.
- **KPI consolidation:** Removed two redundant KPI sections (the old "EXECUTIVE KPI MATRIX" 5-card grid + "STATS CARDS" 4-card grid). Replaced with a single 4-card `StatCard` grid (reused from Manager's `src/features/dashboard/components/StatCard.tsx`) + a 4-card "Attention Center" using semantic accents: RED (overdue/arrears), AMBER (pending/expiring), GREEN (portfolio health), BLUE (maintenance). No invented metrics — all values come from the existing `landlord-portfolio-deep` query.
- **Backend data:** Every displayed value comes from real authorized landlord data via the existing Supabase queries (`property_landlords`, `properties`, `units`, `invoices`, `leases`, `maintenance_requests`, `payout_requests`, `get_landlord_revenue` / `get_landlord_property_revenue_summary` RPCs). Loading/real-zero/no-data/error states distinguished (skeletons, empty states, retry banners). No fake values. RLS/authorization preserved.
- **Components improved:** Header (compact light, identity, account controls), executive page-header band with greeting, KPI grid, Attention Center, Property Portfolio cards (occupancy bars with semantic colors, status badges, responsive flex layout), Activity feed (TIME/EVENT/RESOURCE/STATUS, loading skeleton, "No recent activity." empty state), Payouts table (overflow-x-auto), Financials/Messages/Settings/Documents tabs (section-title headers). `LandlordDocuments` icon colors migrated from hardcoded (`text-green-600`/`text-red-600`/`text-amber-600`) to semantic tokens.
- **Icons:** Standardized across the portal — Building (properties), Home (occupancy/units), Wallet (revenue/share), Wrench (maintenance), CreditCard (payouts), BarChart3 (financials), Shield (landlord identity), Activity (activity feed). Consistent size/stroke/container. Removed unused icon imports (TrendingUp/DollarSign/Users).
- **Verified:** `tsc --noEmit` clean, `eslint src/features/landlord` 0 errors, `vitest run` 690/690 pass (38 files, incl. landlord-access isolation 6/6), `vite build` success, dev server HTTP 200 on `/landlord/login` + `/landlord/dashboard`. Browser-verified: empty portfolio renders correct real-zero state with next-steps empty state; demo landlord (no linked props) shows real zeros, not fake data.
