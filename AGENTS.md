...
- **Remaining bottlenecks**: (1) 230 KB CSS — Tailwind v4 single sheet, purge already applied; splitting would hurt caching. (2) vendor-charts (127 KB gz) loads on first dashboard render — acceptable, cached after. (3) `calqulus-banner.jpg` corrupt + oversized if ever wired in. (4) `resendVerification.test.tsx` flaky under full-suite parallelism (pre-existing).
- Gates: unit 1148 passed / 1 skipped; E2E (a11y + responsive) 25 passed; lint 0 errors; typecheck + build clean.
...
- `/agency/onboarding` rebuilt on **AgencyLayout** (navy + white desk, 2px amber `#C08A37` accent bar, amber only on small icons ŌĆö never fills). Previously it mounted the generic manager `Layout` (blue accent, wrong sidebar).
...
- **Portfolio**: totals strip (Properties, Units, Occupancy, Collected this month, Outstanding [destructive when >0], Clients) + performance-bar column (amber portal-accent width vs max collected) ŌĆö table-based, no card overload.
