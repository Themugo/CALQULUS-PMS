...
- **Asset-store uploads**: `contentService.uploadLandingAsset(file, folder?)` uploads to the **`landing-images`** storage bucket (cache-control 1yr, sanitized path) and returns the public URL. Static adapter returns `{ ok:false }` (never pretends). Editor gained an `AssetField` (paste URL *or* upload) on capability images. Bucket must be created in the Supabase Storage UI; upload fails gracefully if absent.
