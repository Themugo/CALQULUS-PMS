...
CREATE INDEX IF NOT EXISTS payment_receipts_payer_idx ON public.payment_receipts(payer_party_id);
...
BEGIN
  ...
  INSERT INTO public.payment_receipts(transaction_id,receipt_number,payer_party_id,total_amount) VALUES(v_tx,v_receipt_no,p_payer_party_id,p_amount) RETURNING id INTO v_receipt;
...
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
  ...
  ) FROM public.payment_receipts r LEFT JOIN public.payment_parties pp ON pp.id=r.payer_party_id JOIN public.payment_transactions pt ON pt.id=r.transaction_id WHERE r.id=p_receipt_id;
$$;
...
BEGIN
  ...
  FROM public.payment_receipt_recipients pr JOIN public.payment_receipts r ON r.id=pr.receipt_id JOIN public.payment_transactions pt ON pt.id=r.transaction_id
...
DROP POLICY IF EXISTS payment_receipts_recipient_read ON public.payment_receipts;
CREATE POLICY payment_receipts_recipient_read ON public.payment_receipts FOR SELECT USING (EXISTS (SELECT 1 FROM public.payment_receipt_recipients pr WHERE pr.receipt_id=payment_receipts.id AND pr.recipient_user_id=auth.uid()));
