    ...
    'unit_opportunities', COALESCE((
      ...
      FROM (
        SELECT u.id,u.property_id,p.name property_name,u.unit_number,
          ...
          CASE WHEN lower(coalesce(u.status,''))='vacant' THEN 'Vacancy'`r`n               WHEN l.id IS NOT NULL AND l.end_date BETWEEN current_date AND current_date + 90 THEN 'Renewal due'
