...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (phone) {
      // Escape user-controlled values for SMS/WhatsApp (URL encoding)
      ...
      const smsMessage = `Hi ${escapedTenantName}! You've been invited to join ${escapedPropertyName}${escapedUnit ? ` (Unit ${escapedUnit})` : ""} on CALQULUS RMS. Create your account: ${invitationUrl}`;
      ...
      const whatsappResult = await sendWhatsApp(phone, escapedTenantName, escapedPropertyName, escapedUnit, invitationUrl);
    ...
    // Build the email HTML with proper HTML escaping for user-controlled values
    ...
    const emailHtml = `
      ...
      <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #ffffff; padding: 30px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 12px 12px;">
          <h2 style="color: #333; margin-top: 0;">Hi ${safeTenantName}!</h2>
          
          <p style="color: #555; font-size: 16px;">
            <strong>${safeManagerName}</strong> has invited you to join <strong>${safePropertyName}</strong>${safeUnit ? ` (Unit ${safeUnit})` : ""} as a tenant on CALQULUS RMS.
          </p>
