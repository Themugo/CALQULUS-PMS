...
async function sendSMS(phone: string, message: string): Promise<{ success: boolean; error?: string }> {
  if (!AFRICASTALKING_API_KEY || !AFRICASTALKING_USERNAME) {
    logStep("SMS skipped - Africa's Talking not configured");
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const appUrl = Deno.env.get("SITE_URL") || "https://rentflow.ink";
    ...
    const emailResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: Deno.env.get("RESEND_FROM_EMAIL") || "RentFlow <onboarding@resend.dev>",
        to: [email],
