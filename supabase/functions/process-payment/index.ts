...
const notify = async (
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      throw new Error(`${fn} returned ${resp.status}: ${errText.slice(0, 200)}`);
    }
  } catch (e) {
...
serve(async (req) => {
  ...
  try {
    ...
    const [profileRes, companyRes] = await Promise.all([
      supabase.from("profiles").select("full_name, email, phone").eq("id", tenantId).maybeSingle(),
      supabase.from("company_settings").select("company_name, logo_url, currency").maybeSingle(),
    ]);
    const tenant = profileRes.data as { full_name: string; email: string; phone: string } | null;
    ...
    try {
      const safeRef = (() => { try { return (error as any)?.reference ?? null; } catch { return null; } })();
