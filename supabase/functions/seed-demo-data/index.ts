...
serve(async (req) => {
  ...
  const DEMO_SECRET = Deno.env.get("DEMO_SECRET");
  if (!DEMO_SECRET) {
  ...
  try {
    if (action === "reset") {
      ...
      return Response.json({ ok: true, message: "Demo data reset complete" }, { headers: getCorsHeaders(req) });
    }
...
async function seedDemoData(supabase: any) {
  ...
  await supabase.from("submanager_permissions").upsert(
    { submanager_user_id: IDS.agent, manager_id: IDS.manager, can_manage_tenants: true, can_manage_maintenance: true, can_record_payments: false, can_view_financials: true, restrict_to_assigned_properties: false },
